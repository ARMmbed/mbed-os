...
void port_init(port_t *obj, PortName port, int mask, PinDirection dir)
{
    ...
    GPIO_T* gpio =  (GPIO_T*)gpio_clock_enable(port_index);

    obj->port      = port;
    obj->mask      = mask;
    obj->direction = dir;
    obj->reg_in    = gpio->IDATA;
    obj->reg_out   = gpio->ODATA;

    port_dir(obj, dir);
}

 /*!
 * @brief     Set port direction (in/out)
 *
...
void port_write(port_t *obj, int value)
{
 ...
 /*!
 * @brief     Read the current value on the port
 *
 * @param     obj : The port object
 *
 * @retval    An integer with each bit corresponding to an associated port pin setting
 */
