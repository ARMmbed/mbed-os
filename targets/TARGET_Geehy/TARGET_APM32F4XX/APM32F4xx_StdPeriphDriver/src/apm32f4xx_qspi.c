...
void QSPI_Reset(void)
{
    ...
    QSPI1->IOSW = QSPI_IOSW_RESET_VALUE;
    QSPI1->SSIEN = QSPI_SSIEN_RESET_VALUE;
    QSPI1->INTEN = QSPI_INTEN_RESET_VALUE;
    dummy = QSPI1->ICF;
    QSPI1->CTRL1 = QSPI_CTRL1_RESET_VALUE;
    QSPI1->CTRL2 = QSPI_CTRL2_RESET_VALUE;
    QSPI1->CTRL3 = QSPI_CTRL3_RESET_VALUE;
    QSPI1->SLAEN = QSPI_SLAEN_RESET_VALUE;
    QSPI1->BR = QSPI_BR_RESET_VALUE;
    QSPI1->TFL = QSPI_TFL_RESET_VALUE;
    QSPI1->RFL = QSPI_RFL_RESET_VALUE;
    QSPI1->TFTL = QSPI_TFTL_RESET_VALUE;
    QSPI1->RFTL = QSPI_RFTL_RESET_VALUE;
    QSPI1->STS = QSPI_STS_RESET_VALUE;
    QSPI1->RSD = QSPI_RSD_RESET_VALUE;
...
{
    QSPI1->CTRL1_B.CPHA = qspiConfig->clockPhase;
    QSPI1->CTRL1_B.CPOL = qspiConfig->clockPolarity;
    QSPI1->CTRL1_B.FRF = qspiConfig->frameFormat;
    QSPI1->CTRL1_B.DFS = qspiConfig->dataFrameSize;;
    QSPI1->CTRL1_B.SSTEN = qspiConfig->selectSlaveToggle;

    QSPI1->BR = qspiConfig->clockDiv;
}
...
{
    return (uint8_t)QSPI1->TFL_B.TFL;
}
...
{
    return (uint8_t)QSPI1->RFL_B.RFL;
}
...
{
    QSPI1->INTEN &= (uint32_t)~interrupt;
}
