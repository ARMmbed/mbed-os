...
#define LP_APP_TIM_ID          3   // the G-Timer ID for Application
...
uint32_t lp_ticker_read(void)
{
    ...
    //tick_cnt = tick_cnt;   // it's a up counter
...
uint32_t lp_ticker_read(void)
{
    ...
    static uint64_t us_tick;
    s_tick = HalTimerOp.HalTimerReadCount(LP_SYS_TIM_ID);
    do {
        us_tick = s_tick;
        s_tick = HalTimerOp.HalTimerReadCount(LP_SYS_TIM_ID);
    } while (us_tick != s_tick);
    us_tick = us_tick / (GTIMER_CLK_HZ/1000000);
    return ((uint32_t)us_tick);
}
#endif
