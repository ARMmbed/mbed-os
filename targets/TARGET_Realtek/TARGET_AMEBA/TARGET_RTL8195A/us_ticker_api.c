...
#define US_APP_TIM_ID          4   // the G-Timer ID for Application
...
uint32_t TICK_TO_US(uint32_t zzw_cur_time_us)
{
    static uint32_t ticks_125ms;
    static uint32_t ticks_remain;
    static uint64_t us_tick;

    ticks_125ms = zzw_cur_time_us/(GTIMER_CLK_HZ/8);
    ticks_remain = zzw_cur_time_us - (ticks_125ms*(GTIMER_CLK_HZ/8));
...
void us_ticker_set_interrupt(timestamp_t timestamp) 
{
    ...
    cur_time_us = TICK_TO_US(0xFFFFFFFFUL - cur_time_us);
