...
void serial_init(serial_t *obj, PinName tx, PinName rx)
{
    ...
    switch (uart_n) {
        case 0:
            obj->uart = (LPC_USART0_Type *)LPC_USART0_BASE;
            break;
        case 1:
            obj->uart = (LPC_USART0_Type *)LPC_USART1_BASE;
            break;
        case 2:
            obj->uart = (LPC_USART0_Type *)LPC_USART2_BASE;
            break;
    }
    uart_used |= (1 << uart_n);
...
void uart0_irq()
{
    uart_irq((LPC_USART0->INTSTAT & 1) ? RxIrq : TxIrq, 0);
}
void uart1_irq()
{
    uart_irq((LPC_USART1->INTSTAT & 1) ? RxIrq : TxIrq, 1);
}
void uart2_irq()
{
    uart_irq((LPC_USART2->INTSTAT & 1) ? RxIrq : TxIrq, 2);
}
...
{
    if ((FlowControlNone == type || FlowControlRTS == type)) {
        txflow = NC;
    }
    if ((FlowControlNone == type || FlowControlCTS == type)) {
        rxflow = NC;
    }
