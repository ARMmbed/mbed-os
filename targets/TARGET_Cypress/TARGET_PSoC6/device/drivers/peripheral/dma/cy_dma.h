...
*
* The DMA Channel component can be used in any project that needs to transfer data
...
* Many API functions for the DMA driver require the base hardware address and the 
...
*
* To set up a DMA driver you initialize a descriptor,
...
* 
* To set up the DMA channel, provide a filled \ref cy_stc_dma_channel_config_t 
...
*
* The following is a simplified structure of the DMA driver API interdependences
...
* See also the DMA chapter of the device technical reference manual (TRM).
*
...
*     <td>14.2</td>
*     <td>R</td>
*     <td>All non-null statements shall either:
*       a) have at least one side effect however executed
*       b) cause control flow to change</td>
*     <td>A result of a read operation is ignored when this read is just intended
...
*         * The descriptor API is update in the next manner:
*           The \ref Cy_DMA_Descriptor_Init function sets whole the bunch of descriptor
*           settings, and all the rest of descriptor API is the get/set interface
*           to the each of descriptor settings.
*         * There is a bunch of macros to support a backward compatibility with most 
*           of the driver version 1.0 API, however it is strongly recommended to use
...
*           definition should be changed to "true".</td>
*     <td></td>
*   </tr>
* </table>
*
...
/** DMA driver identifier */
#define CY_DMA_ID                      (CY_PDL_DRV_ID(0x13U))
 
/** DMA channel interrupt mask */
#define CY_DMA_INTR_MASK               (0x01UL)

/** Backward compatibility flag. It enables a bunch of macros which provide 
* a backward compatibility with most of DMA driver version 1.0 interface. */
#ifndef CY_DMA_BWC
    #define CY_DMA_BWC                 (false)
#endif
...
typedef enum
{
    CY_DMA_RETDIG_IM    = 0UL,             /**< Retrigger immidiatelly         */
    CY_DMA_RETDIG_4CYC  = 1UL,             /**< Retrigger after 4 SYSCLK       */
    CY_DMA_RETDIG_16CYC = 2UL,             /**< Retrigger after 16 SYSCLK      */
...
typedef enum 
{
    ...
    CY_DMA_BAD_PARAM        = CY_DMA_ID | CY_PDL_STATUS_ERROR | 0x01UL /**< Input parameters passed to DMA API are not valid. */
} cy_en_dma_status_t;
...
#define CY_DMA_IS_RETRIGGER_VALID(retrigger)  ((CY_DMA_RETDIG_IM      == (retrigger)) || \
                                               (CY_DMA_RETDIG_4CYC    == (retrigger)) || \
                                               (CY_DMA_RETDIG_16CYC   == (retrigger)) || \
...
/* Descriptor structure bitfiled definitions */
...
/** 
* DMA descriptor structure type. It is a user/component-declared structure which is
* allocated in RAM and just a pointer to which is needed for DMA HW to work with it.
*
* For advanced users: the descriptor can be allocated even in Flash, but in this
* case user should manually predefine all the structure items with constants, 
* bacause most of driver's API (especially those functions which are modifying
...
* In case of RAM allocation there is a possibility to reinitialize a descriptor in runtime.
...
__STATIC_INLINE void Cy_DMA_Descriptor_SetDescriptorType   (cy_stc_dma_descriptor_t * descriptor, cy_en_dma_descriptor_type_t descriptorType);
...
*
* \param dataSize
* Data element size \ref cy_en_dma_data_size_t
*  
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Descriptor_SetDataSize(cy_stc_dma_descriptor_t * descriptor, cy_en_dma_data_size_t dataSize)
{
...
*
* \return
* Source Transfer Size \ref cy_en_dma_transfer_size_t.
*  
*******************************************************************************/
__STATIC_INLINE cy_en_dma_transfer_size_t Cy_DMA_Descriptor_GetSrcTransferSize(cy_stc_dma_descriptor_t const * descriptor)
{
...
* A Destination Transfer Size \ref cy_en_dma_transfer_size_t.
*
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Descriptor_SetDstTransferSize(cy_stc_dma_descriptor_t * descriptor, cy_en_dma_transfer_size_t dstTransferSize)
{
...
*
* \param retrigger 
* A \ref cy_en_dma_retrigger_t parameter specifies whether the controller
...
* During the descriptor's type changing the Next Descriptor parameter should be 
* properly replaced by \ref Cy_DMA_Descriptor_GetNextDescriptor
* (before reconfiguration) and \ref Cy_DMA_Descriptor_SetNextDescriptor (before
* reconfiguration) API functions.
* 
*
* \param descriptor
* A descriptor structure instance declared by user/component.
...
* A Descriptor Type \ref cy_en_dma_descriptor_type_t.
*
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Descriptor_SetDescriptorType(cy_stc_dma_descriptor_t * descriptor, cy_en_dma_descriptor_type_t descriptorType)
{
...
*
* \param xCount
* A number of data elements to transfer in the X loop.
*  
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Descriptor_SetXloopDataCount(cy_stc_dma_descriptor_t * descriptor, uint32_t xCount)
{
...
__STATIC_INLINE uint32_t Cy_DMA_Descriptor_GetXloopDataCount(cy_stc_dma_descriptor_t const * descriptor)
{
    ...
    return (_FLD2VAL(CY_DMA_CTL_COUNT, descriptor->xCtl));
}
...
*
* \param srcXincrement
* A value of source increment. Valid range is -2048 ... 2047.
*
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Descriptor_SetXloopSrcIncrement(cy_stc_dma_descriptor_t * descriptor, int32_t srcXincrement)
{
...
*
* \param dstXincrement
* A value of destination increment. Valid range is -2048 ... 2047.
*  
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Descriptor_SetXloopDstIncrement(cy_stc_dma_descriptor_t * descriptor, int32_t dstXincrement)
{
...
*
* \return
* The value of destination increment.
*  
*******************************************************************************/
__STATIC_INLINE int32_t Cy_DMA_Descriptor_GetXloopDstIncrement(cy_stc_dma_descriptor_t const * descriptor)
{
...
*
* \param yCount
* A number of X loops to execute in the Y loop.
*  
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Descriptor_SetYloopDataCount(cy_stc_dma_descriptor_t * descriptor, uint32_t yCount)
{
...
__STATIC_INLINE uint32_t Cy_DMA_Descriptor_GetYloopDataCount(cy_stc_dma_descriptor_t const * descriptor)
{
    ...
    return (_FLD2VAL(CY_DMA_CTL_COUNT, descriptor->yCtl));
}
...
****************************************************************************//**
*
* Sets a descriptor as current for specified DMA channel.
*
* \param base
* A pointer to the hardware DMA block.
*
...
****************************************************************************//**
*
* Returns the priority of DMA channel.
*
* \param base
* A pointer to the hardware DMA block.
*
...
*
* \return
* The pointer to the descriptor that assocaited with the channel.
*
*******************************************************************************/
__STATIC_INLINE cy_stc_dma_descriptor_t * Cy_DMA_Channel_GetCurrentDescriptor(DW_Type const * base, uint32_t channel)
{
...
*
* \return
* The status of an interrupt for specified channel.
*
*******************************************************************************/
__STATIC_INLINE uint32_t Cy_DMA_Channel_GetInterruptStatus(DW_Type const * base, uint32_t channel)
{
...
*
* \return
* A cause \ref cy_en_dma_intr_cause_t of an interrupt.
*
*******************************************************************************/
__STATIC_INLINE cy_en_dma_intr_cause_t Cy_DMA_Channel_GetStatus(DW_Type const * base, uint32_t channel)
{
...
****************************************************************************//**
*
* Returns interrupt mask value of the specified channel.
*
* \param base
* A pointer to the hardware DMA block.
*
...
****************************************************************************//**
*
* Sets interrupt mask value for the specified channel.
*
* \param base
* A pointer to the hardware DMA block.
*
...
* \param interrupt
* An interrupt mask:
* CY_DMA_INTR_MASK to enable interrupt or 0UL to disable interrupt.
*
*******************************************************************************/
__STATIC_INLINE void Cy_DMA_Channel_SetInterruptMask(DW_Type * base, uint32_t channel, uint32_t interrupt)
{
...
****************************************************************************//**
*
* Returns logical AND of corresponding INTR and INTR_MASK fields
...
/** \cond Definitions to support backward compatibility, they are strongly not recommended to use in new designs */
...
* It is strongly not recommended to use in new designs.
*******************************************************************************/
