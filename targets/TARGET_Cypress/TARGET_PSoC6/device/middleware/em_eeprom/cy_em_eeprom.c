...
static void GetNextRowToWrite(uint32_t seqNum,
        uint32_t * rowToWrPtr, uint32_t * rowToRdPtr, cy_stc_eeprom_context_t * context);
...
static cy_en_em_eeprom_status_t CheckCrcAndCopy(uint32_t startAddr,
    uint32_t dstAddr, uint32_t rowOffset, uint32_t numBytes, cy_stc_eeprom_context_t * context);
...
*******************************************************************************/
cy_en_em_eeprom_status_t
    Cy_Em_EEPROM_Read(uint32_t addr, void * eepromData, uint32_t size, cy_stc_eeprom_context_t * context)
{
    cy_en_em_eeprom_status_t ret = CY_EM_EEPROM_BAD_PARAM;
    uint32_t i;
    ...
    if((0u != size) && ((addr + size) <= (context->eepromSize)) && (NULL != eepromData))
    {
        ...
        uint32_t seqNum = CY_GET_REG32(context->lastWrRowAddr);
                ...
                for(i = (seqNum - numRowReads); i <= seqNum; i++)
                {
                    ...
                    if (0u != startRowAddr)
                    {
                        ...
                        if(0u != CY_EM_EEPROM_IS_ADDRESES_CROSSING(startAddr, endAddr, addr, addr + size))
                        {
                            ...
                            eeData = ((uint32_t)eepromData) + (rdAddr - addr);
...
*******************************************************************************/
cy_en_em_eeprom_status_t
    Cy_Em_EEPROM_Write(uint32_t addr, void * eepromData, uint32_t size, cy_stc_eeprom_context_t * context)
{
    cy_en_em_eeprom_status_t ret = CY_EM_EEPROM_BAD_PARAM;
    uint32_t i;
    uint32_t wrCnt;
    ...
    if((0u != size) && ((addr + size) <= (context->eepromSize)) && (NULL != eepromData))
    {
        ...
        for(wrCnt = 0u; wrCnt < numWrites; wrCnt++)
        {
            ...
            /* Get the sequence number of the last written row */
            uint32_t seqNum = CY_GET_REG32(emEepromRowAddr);
...
*
*******************************************************************************/
static cy_en_em_eeprom_status_t WriteRow(uint32_t rowAddr, uint32_t *rowData,
...
*
*******************************************************************************/
static cy_en_em_eeprom_status_t EraseRow(uint32_t rowAddr, uint32_t ramBuffAddr,
    cy_stc_eeprom_context_t * context)
{
...
*
*******************************************************************************/
static uint32_t GetAddresses(uint32_t *startAddr, uint32_t *endAddr, uint32_t *offset, uint32_t rowNum,
