...
#include "s5js100_network_data_statistics.h"

#define TRACE_GROUP             "S5JS100_L3IP"
#define S5JS100_BUFF_ALIGNMENT  0

s5j_data_statistics_t s5j_data_statistics;
...
bool SAMSUNG_S5JS100_L3IP_Driver::link_out(net_stack_mem_buf_t *buf)
{
    ...
    if (cont_buf)
    {
        ...
        if (sent != total_len)
        {
            ...
            s5j_data_statistics.bytesTxed += sent;
        }
...
void SAMSUNG_S5JS100_L3IP_Driver::packet_rx(mio_buf * buf)
{
    ...
    if (read_buf)
    {
        ...
        s5j_data_statistics.bytesRxed += buf->len;
