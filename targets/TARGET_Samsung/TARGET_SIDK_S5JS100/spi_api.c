...
#if 0
	unsigned int ch_cfg;
	ch_cfg = getreg32(&(obj->spi->CONFIG));
	ch_cfg = (ch_cfg & (~CH_CFG_MODE_MASK)) | CH_CFG_MODE(mode);
	putreg32(ch_cfg, &(obj->spi->CONFIG));
#else
...
void spi_setbits(spi_t *obj, int nbits)
{
	...
	mode_cfg = getreg32(&obj->spi->MODE_CONFIG);
	...
	default:
		error("SPI bits setting failed");
		break;
	}

	putreg32(mode_cfg, &obj->spi->MODE_CONFIG);
#else
...
static void spi_exchange(spi_t *obj, const void *txbuffer, void *rxbuffer, unsigned int nwords)
{
	...
	/* Here I read FIFO bus width from SPI register */
	int fifo_width = GET_MODE_CFG_BUSW(pSPIRegs->MODE_CONFIG);

	/* SPI FIFO FLUSH */
	int ch_cfg = pSPIRegs->CONFIG;

	pSPIRegs->CONFIG = CH_CFG_FIFO_FLUSH;
	pSPIRegs->CONFIG = ch_cfg;
	...
	if ((rxbuffer == NULL) && (txbuffer == NULL)) {
		while (received < nwords) {
			if (sent < nwords)
				if (SPI_STAT_TX_FIFO_LVL(pSPIRegs->SPI_STATUS) < 64) {
					pSPIRegs->SPI_TX_DATA = 0;
					sent++;
				}

			if (SPI_STAT_RX_FIFO_LVL(pSPIRegs->SPI_STATUS) > 0) {
				dummy_rx = pSPIRegs->SPI_RX_DATA;
				received++;
			}
		}
		return;
	...
	if (rxbuffer == NULL) {
		while (received < nwords) {
			if (sent < nwords)
				if (SPI_STAT_TX_FIFO_LVL(pSPIRegs->SPI_STATUS) < 64) {
					if (fifo_width == 1) {
						pSPIRegs->SPI_TX_DATA = ((unsigned char *) txbuffer)[sent];
					} else if (fifo_width == 2) {
						pSPIRegs->SPI_TX_DATA = ((unsigned short *) txbuffer)[sent];
					} else {
						pSPIRegs->SPI_TX_DATA = ((unsigned int *) txbuffer)[sent];
					}
					sent++;
				}
	...
	if (txbuffer == NULL) {
		while (received < nwords) {
			if (sent < nwords)
				if (SPI_STAT_TX_FIFO_LVL(pSPIRegs->SPI_STATUS) < 64) {
					if (fifo_width == 1) {
						pSPIRegs->SPI_TX_DATA = ((unsigned char *) rxbuffer)[sent];
					} else if (fifo_width == 2) {
						pSPIRegs->SPI_TX_DATA = ((unsigned short *) rxbuffer)[sent];
					} else {
						pSPIRegs->SPI_TX_DATA = ((unsigned int *) rxbuffer)[sent];
					}
					sent++;
				}
			...
			if (SPI_STAT_RX_FIFO_LVL(pSPIRegs->SPI_STATUS) > 0) {
				if (fifo_width == 1) {
					((unsigned char *) rxbuffer)[received] = pSPIRegs->SPI_RX_DATA;
				} else if (fifo_width == 2) {
					((unsigned short *) rxbuffer)[received] = pSPIRegs->SPI_RX_DATA;
				} else {
					((unsigned int *) rxbuffer)[received] = pSPIRegs->SPI_RX_DATA;
				}
				received++;
			}
		}
