...
#if 0
	unsigned int i;
	for (i = 0; i < buf_len; i++) up_putc(buf[i]);
#endif
}
...
static void convert2hex(char* _buffer, unsigned int data)
{
	...
	if (_buffer[6] > '9') _buffer[6] += 7;
	if (_buffer[7] > '9') _buffer[7] += 7;
	if (_buffer[8] > '9') _buffer[8] += 7;
	if (_buffer[9] > '9') _buffer[9] += 7;
}
#endif
...
{
	modifyreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio, (0x3 << 0), 1 << cfg);
}

void alive_gpio_set_value(PORT_NUM gpio, int value)
{
	modifyreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio, (0x1 << 5), value << 5);
}

int alive_gpio_get_value(PORT_NUM gpio)
{
	return ((getreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio) >> 4) & 0x1);
}

void gpio_set_pull(PORT_NUM gpio, PUD_CON mode)
{
	modifyreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio, (0x3 << 2), mode << 2);
}

void gpio_eint_mask(PORT_NUM gpio)
{
	modifyreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio, (1u << 9), 0x0 << 9);
}

void gpio_eint_unmask(PORT_NUM gpio)
{
	modifyreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio, (1u << 9), 0x1 << 9);
}

int gpio_eint_is_pending(PORT_NUM gpio)
{
	return ((getreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio) >> 10) & 0x1);
}

void gpio_eint_clear_pending(PORT_NUM gpio)
{
	modifyreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio, (1u << 10), 1 << 10);
}

void gpio_eint_set_filter(PORT_NUM gpio, FILTER_TYPE type)
{
	modifyreg32(PMU_ALIVE_GPIOALV0CTRL + 0x4 * gpio, (0x3 << 6), type << 6);
}
...
static int alvgpio0_interrupt(void)
{
	if (getreg32(0x810000f0) & (0x1 << 4)) {
		gpio_eint_set_filter(GPIO_ALIVE_0, LOWLEVEL);
		external_pin = 1;
	} else {
		gpio_eint_set_filter(GPIO_ALIVE_0, HIGHLEVEL);
		external_pin = 0;
	}
...
void alvgpio0_interrupt_enable(void)
{
	NVIC_SetVector(S5JS100_IRQ_PMU_AlivePad, (unsigned int)alvgpio0_interrupt);
#if defined (__ICACHE_PRESENT) && (__ICACHE_PRESENT == 1U)
		SCB_InvalidateICache();
#endif
	gpio_eint_unmask(GPIO_ALIVE_0);	//IntEn=1
	NVIC_EnableIRQ(S5JS100_IRQ_PMU_AlivePad);
}

void alvgpio0_init(void)
{
	modifyreg32(0x85023088, 0x1, 0x1);
	gpio_set_pull(GPIO_ALIVE_0, PULL_DOWN);
	if (getreg32(0x810000f0) & (0x1 << 4)) {
		gpio_eint_set_filter(GPIO_ALIVE_0, LOWLEVEL);
		external_pin = 1;
	} else {
...
void s5js100_pmu_init(void)
{
	...
	alvgpio0_init();
	alvgpio0_interrupt_enable();

	// GpioAlv1Ctrl set high, alv gpio0 IE = 0
	putreg32((1u << 0) | (1u << 5), 0x810000f4);
	modifyreg32(0x810000f0, 0x3, 0x2);
...
void s5js100_pmu_sleep(PMU_SLEEP_INFO *info)
{
 ...
 #warning "no support s5js100_sw_powerdomain_off"
	...
	if ((_wkupsrc & GPIO_ALV0) > 0) {	//active high
		gpio_eint_clear_pending(GPIO_ALIVE_0);
		//      putreg32((1u << 6) + (1u << 2) + (1u << 1), PMU_ALIVE_GPIOALV0CTRL);    //Polarity=1(active high) default input-pulldown. active high
		if ((pmu_info->alv0_gpio_int_type == LOWLEVEL) || (pmu_info->alv0_gpio_int_type == FALLING)) {
			gpio_set_pull(GPIO_ALIVE_0, PULL_UP);
			//gpio_set_pull(GPIO_ALIVE_0, PULL_DIS);
		} else {
			gpio_set_pull(GPIO_ALIVE_0, PULL_DOWN);
			//gpio_set_pull(GPIO_ALIVE_0, PULL_DIS);
		}
		gpio_eint_set_filter(GPIO_ALIVE_0, pmu_info->alv0_gpio_int_type);

		gpio_eint_unmask(GPIO_ALIVE_0);	//IntEn=1
	}

	if ((_wkupsrc & GPIO_ALV1) > 0) {	//active low, pulse type
		//      putreg32((1u << 7) + (1u << 3) + (1u << 2) + (1u << 1), PMU_ALIVE_GPIOALV1CTRL);    //Type=1(edge), Polarity=0(active low), PS=1, PE=1,IE=1
		if ((pmu_info->alv1_gpio_int_type == LOWLEVEL) || (pmu_info->alv1_gpio_int_type == FALLING)) {
			gpio_set_pull(GPIO_ALIVE_1, PULL_UP);
			//gpio_set_pull(GPIO_ALIVE_1, PULL_DIS);
		} else {
			gpio_set_pull(GPIO_ALIVE_1, PULL_DOWN);
			//gpio_set_pull(GPIO_ALIVE_1, PULL_DIS);
		}
		gpio_eint_set_filter(GPIO_ALIVE_1, pmu_info->alv1_gpio_int_type);

		gpio_eint_unmask(GPIO_ALIVE_1);	//IntEn=1
	}
...
void pmu_nb_sleep(void)
{
	...
	pmu_info.wakeup_src = NSLEEP | GPIO_ALV0;
	pmu_info.alv0_gpio_int_type = HIGHLEVEL;
...
void pmu_short_sleep(void)
{
 ...
 #warning "ICCARM no support idle drx"
	...
	gpio_eint_mask(GPIO_ALIVE_0);	//IntEn=1
	alvgpio0_interrupt_enable();
}
