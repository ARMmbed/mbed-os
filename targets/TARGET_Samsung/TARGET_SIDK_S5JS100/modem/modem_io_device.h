...
public:
	...
	virtual int read(char *buffer, int buflen) { return 0;}
	int write(const char *buffer, int buflen);
	virtual int poll(int setup) { return 0; }
	int ioctl(int cmd, unsigned long arg);
	void register_ReadCb(void (*cb)(mio_buf *, void *), void *data) { read_cb = cb; cb_data = data; }
