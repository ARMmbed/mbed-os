...
/**
  * @brief      This function makes USBD module to be ready to use API for Non-Secure
  */
...
/**
  * @brief    This function makes USB host to recognize the device API for Non-Secure
  */
...
//    CLK_SysTickDelay(100000);
    ...
    /* Clear USB-related interrupts before enable interrupt */
    USBD_CLR_INT_FLAG_NS(USBD_INT_BUS | USBD_INT_USB | USBD_INT_FLDET | USBD_INT_WAKEUP);

    /* Enable USB-related interrupts. */
    USBD_ENABLE_INT_NS(USBD_INT_BUS | USBD_INT_USB | USBD_INT_FLDET | USBD_INT_WAKEUP);
}
...
/**
  * @brief    Process SETUP packet API for Non-Secure
  */
void USBD_ProcessSetupPacket_NS(void)
{
    /* Get SETUP packet from USB buffer */
    USBD_MemCopy(g_usbd_SetupPacket, (uint8_t *)USBD_BUF_BASE_NS, 8);
    ...
    switch(g_usbd_SetupPacket[0] & 0x60)
    {
        ...
        {
            /* Setup error, stall the device */
            USBD_SET_EP_STALL_NS(EP0);
            USBD_SET_EP_STALL_NS(EP1);
            break;
        }
    }
}
...
/**
  * @brief    Process GetDescriptor request API for Non-Secure
  */
void USBD_GetDescriptor_NS(void)
{
    uint32_t u32Len;
    ...
    switch(g_usbd_SetupPacket[3])
    {
        ...
        case DESC_DEVICE:
        {
            ...
            USBD_PrepareCtrlIn_NS((uint8_t *)g_usbd_sInfo->gu8DevDesc, u32Len);
        ...
        case DESC_CONFIG:
        {
            ...
            USBD_PrepareCtrlIn_NS((uint8_t *)g_usbd_sInfo->gu8ConfigDesc, u32Len);
        ...
        USBD_PrepareCtrlIn_NS((uint8_t *)g_usbd_sInfo->gu8BosDesc, u32Len);
        break;
        }
        // Get HID Descriptor
        case DESC_HID:
        {
            ...
            u32ConfigDescOffset = g_usbd_sInfo->gu32ConfigHidDescIdx[g_usbd_SetupPacket[4]];
            USBD_PrepareCtrlIn_NS((uint8_t *)&g_usbd_sInfo->gu8ConfigDesc[u32ConfigDescOffset], u32Len);
        ...
        case DESC_HID_RPT:
        {
            ...
            u32Len = USBD_Minimum(u32Len, g_usbd_sInfo->gu32HidReportSize[g_usbd_SetupPacket[4]]);
            USBD_PrepareCtrlIn_NS((uint8_t *)g_usbd_sInfo->gu8HidReportDesc[g_usbd_SetupPacket[4]], u32Len);
            break;
        }
        // Get String Descriptor
        case DESC_STRING:
        {
            ...
            if(g_usbd_SetupPacket[2] < 4)
            {
                ...
                USBD_PrepareCtrlIn_NS((uint8_t *)g_usbd_sInfo->gu8StringDesc[g_usbd_SetupPacket[2]], u32Len);
...
void USBD_StandardRequest(void)
{
    ...
    else
    {
        ...
        switch(g_usbd_SetupPacket[1])
        {
            ...
            case SET_FEATURE:
            {
                if((g_usbd_SetupPacket[0] & 0xF) == 0)   /* 0: device */
                {
                    if((g_usbd_SetupPacket[2] == 3) && (g_usbd_SetupPacket[3] == 0)) /* 3: HNP enable */
                    {
...
/**
  * @brief    Process standard request API for Non-Secure
  */
    ...
    if(g_usbd_SetupPacket[0] & 0x80)    /* request data transfer direction */
    {
        ...
        switch(g_usbd_SetupPacket[1])
        {
            case GET_CONFIGURATION:
            {
                // Return current configuration setting
                /* Data stage */
                M8(USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0)) = g_usbd_UsbConfig;
                USBD_SET_DATA1_NS(EP0);
                USBD_SET_PAYLOAD_LEN_NS(EP0, 1);
                /* Status stage */
                USBD_PrepareCtrlOut_NS(0,0);
            ...
            case GET_DESCRIPTOR:
            {
                USBD_GetDescriptor_NS();
                USBD_PrepareCtrlOut_NS(0, 0); /* For status stage */
                break;
            }
            case GET_INTERFACE:
            {
                // Return current interface setting
                /* Data stage */
                M8(USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0)) = g_usbd_UsbAltInterface;
                USBD_SET_DATA1_NS(EP0);
                USBD_SET_PAYLOAD_LEN_NS(EP0, 1);
                /* Status stage */
                USBD_PrepareCtrlOut_NS(0, 0);
            ...
            case GET_STATUS:
            {
                ...
                if(g_usbd_SetupPacket[0] == 0x80)
                {
                    ...
                    M8(USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0)) = u8Tmp;

                }
                // Interface
                else if(g_usbd_SetupPacket[0] == 0x81)
                    M8(USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0)) = 0;
                // Endpoint
                else if(g_usbd_SetupPacket[0] == 0x82)
                {
                    uint8_t ep = g_usbd_SetupPacket[4] & 0xF;
                    M8(USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0)) = USBD_GetStall_NS(ep) ? 1 : 0;
                }

                M8(USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0) + 1) = 0;
                /* Data stage */
                USBD_SET_DATA1_NS(EP0);
                USBD_SET_PAYLOAD_LEN_NS(EP0, 2);
                /* Status stage */
                USBD_PrepareCtrlOut_NS(0, 0);
        ...
        switch(g_usbd_SetupPacket[1])
        {
            case CLEAR_FEATURE:
            {
                if(g_usbd_SetupPacket[2] == FEATURE_ENDPOINT_HALT)
                {
                    ...
                    for(i = 0; i < USBD_MAX_EP; i++)
                    {
                        if(((USBD_NS->EP[i].CFG & 0xF) == epNum) && ((g_u32EpStallLock & (1 << i)) == 0))
                        {
                            USBD_NS->EP[i].CFGP &= ~USBD_CFGP_SSTALL_Msk;
                            DBG_PRINTF("Clr stall ep%d %x\n", i, USBD_NS->EP[i].CFGP);
                        }
                    }
                }
            ...
            case SET_FEATURE:
            {
                if( (g_usbd_SetupPacket[0] & 0xF) == 0 ) /* 0: device */
                {
                    if((g_usbd_SetupPacket[2] == 3) && (g_usbd_SetupPacket[3] == 0)) /* 3: HNP enable */
                    {
                        OTG_NS->CTL |= (OTG_CTL_HNPREQEN_Msk | OTG_CTL_BUSREQ_Msk);
                    }
                }
                if(g_usbd_SetupPacket[2] == FEATURE_ENDPOINT_HALT)
                {
                    USBD_SetStall_NS(g_usbd_SetupPacket[4] & 0xF);
                    DBG_PRINTF("Set feature. stall ep %d\n", g_usbd_SetupPacket[4] & 0xF);
                }
                else if(g_usbd_SetupPacket[2] == FEATURE_DEVICE_REMOTE_WAKEUP)
                {
...
/**
  * @brief      Prepare the first Control IN pipe API for Non-Secure
  */
void USBD_PrepareCtrlIn_NS(uint8_t *pu8Buf, uint32_t u32Size)
{
    ...
    if(u32Size > g_usbd_CtrlMaxPktSize)
    {
        ...
        g_usbd_CtrlInSize = u32Size - g_usbd_CtrlMaxPktSize;
        USBD_SET_DATA1_NS(EP0);
        USBD_MemCopy((uint8_t *)USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0), pu8Buf, g_usbd_CtrlMaxPktSize);
        USBD_SET_PAYLOAD_LEN_NS(EP0, g_usbd_CtrlMaxPktSize);
    }
    else
    {
        // Data size <= MXPLD
        ...
        g_usbd_CtrlInSize = 0;
        USBD_SET_DATA1_NS(EP0);
        USBD_MemCopy((uint8_t *)USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0), pu8Buf, u32Size);
        USBD_SET_PAYLOAD_LEN_NS(EP0, u32Size);
    }
}
...
/**
  * @brief    Repeat Control IN pipe API for Non-Secure
  */
void USBD_CtrlIn_NS(void)
{
    ...
    if(g_usbd_CtrlInSize)
    {
        ...
        {
            // Data size > MXPLD
            USBD_MemCopy((uint8_t *)USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0), (uint8_t *)g_usbd_CtrlInPointer, g_usbd_CtrlMaxPktSize);
            USBD_SET_PAYLOAD_LEN_NS(EP0, g_usbd_CtrlMaxPktSize);
            g_usbd_CtrlInPointer += g_usbd_CtrlMaxPktSize;
            g_usbd_CtrlInSize -= g_usbd_CtrlMaxPktSize;
        }
        ...
        {
            // Data size <= MXPLD
            USBD_MemCopy((uint8_t *)USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP0), (uint8_t *)g_usbd_CtrlInPointer, g_usbd_CtrlInSize);
            USBD_SET_PAYLOAD_LEN_NS(EP0, g_usbd_CtrlInSize);
            if(g_usbd_CtrlInSize == g_usbd_CtrlMaxPktSize)
                u8ZeroFlag = 1;
            g_usbd_CtrlInPointer = 0;
            g_usbd_CtrlInSize = 0;
    ...
    else // No more data for IN token
    {
        ...
        if((g_usbd_SetupPacket[0] == REQ_STANDARD) && (g_usbd_SetupPacket[1] == SET_ADDRESS))
        {
            if((USBD_GET_ADDR_NS() != g_usbd_UsbAddr) && (USBD_GET_ADDR_NS() == 0))
            {
                USBD_SET_ADDR_NS(g_usbd_UsbAddr);
            }
        }
...
/**
  * @brief      Prepare the first Control OUT pipe API for Non-Secure
  */
void USBD_PrepareCtrlOut_NS(uint8_t *pu8Buf, uint32_t u32Size)
{
    ...
    g_usbd_CtrlOutSize = 0;
    g_usbd_CtrlOutSizeLimit = u32Size;
    USBD_SET_PAYLOAD_LEN_NS(EP1, g_usbd_CtrlMaxPktSize);
}
...
/**
  * @brief    Repeat Control OUT pipe API for Non-Secure
  */
void USBD_CtrlOut_NS(void)
{
    ...
    if(g_usbd_CtrlOutSize < g_usbd_CtrlOutSizeLimit)
    {
        u32Size = USBD_GET_PAYLOAD_LEN_NS(EP1);
        USBD_MemCopy((uint8_t *)g_usbd_CtrlOutPointer, (uint8_t *)USBD_BUF_BASE_NS + USBD_GET_EP_BUF_ADDR_NS(EP1), u32Size);
        g_usbd_CtrlOutPointer += u32Size;
        g_usbd_CtrlOutSize += u32Size;
...
/**
  * @brief    Reset software flags API for Non-Secure
  */
void USBD_SwReset_NS(void)
{
    ...
    /* Reset PID DATA0 */
    for(i=0; i<USBD_MAX_EP; i++)
        USBD_NS->EP[i].CFG &= ~USBD_CFG_DSQSYNC_Msk;
