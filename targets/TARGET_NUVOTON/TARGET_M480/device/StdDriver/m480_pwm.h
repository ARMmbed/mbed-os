...
/*---------------------------------------------------------------------------------------------------------*/
#define PWM_DUTY_INT_DOWN_COUNT_MATCH_CMP        (1 << PWM_INTEN0_CMPDIENn_Pos)   /*!< PWM duty interrupt triggered if down count match comparator */
#define PWM_DUTY_INT_UP_COUNT_MATCH_CMP          (1 << PWM_INTEN0_CMPUIENn_Pos)   /*!< PWM duty interrupt triggered if up down match comparator */
...
/*---------------------------------------------------------------------------------------------------------*/
#define PWM_LOAD_MODE_IMMEDIATE                  (1 << PWM_CTL0_IMMLDENn_Pos)    /*!< PWM immediately load mode \hideinitializer */
#define PWM_LOAD_MODE_WINDOW                     (1 << PWM_CTL0_WINLDENn_Pos)    /*!< PWM window load mode \hideinitializer */
#define PWM_LOAD_MODE_CENTER                     (1 << PWM_CTL0_CTRLDn_Pos)      /*!< PWM center load mode \hideinitializer */
 ...
 * \hideinitializer
 */
#define PWM_ENABLE_COMPLEMENTARY_MODE(pwm) ((pwm)->CTL1 = (pwm)->CTL1 | (0x7ul<<PWM_CTL1_OUTMODEn_Pos))
 ...
 * \hideinitializer
 */
#define PWM_DISABLE_COMPLEMENTARY_MODE(pwm) ((pwm)->CTL1 = (pwm)->CTL1 & ~(0x7ul<<PWM_CTL1_OUTMODEn_Pos))
   ...
   do{ \
        ...
        for(i = 0; i < 6; i++) { \
            if((u32ChannelMask) & (1 << i)) { \
                ...
                (pwm)->WGCTL0 = (((pwm)->WGCTL0 & ~(3UL << (PWM_WGCTL0_PRDPCTLn_Pos + (i << 1)))) | ((u32PeriodLevel) << (PWM_WGCTL0_PRDPCTLn_Pos + (i << 1)))); \
                (pwm)->WGCTL1 = (((pwm)->WGCTL1 & ~(3UL << (i << 1))) | ((u32CmpUpLevel) << (i << 1))); \
                (pwm)->WGCTL1 = (((pwm)->WGCTL1 & ~(3UL << (PWM_WGCTL1_CMPDCTLn_Pos + (i << 1)))) | ((u32CmpDownLevel) << (PWM_WGCTL1_CMPDCTLn_Pos + (i << 1)))); \
            } \
        } \
    }while(0)
