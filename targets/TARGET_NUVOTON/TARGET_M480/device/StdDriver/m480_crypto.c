...
#else
#define CRPT_DBGMSG(...)          // disable debug
#endif
...
void PRNG_Read(uint32_t u32RandKey[])
{
    ...
    for (i = 0; i < wcnt; i++)
        u32RandKey[i] = *(uint32_t *)((uint32_t)&(CRPT->PRNG_KEY[0]) + (i * 4));
...
void AES_SetKey(uint32_t u32Channel, uint32_t au32Keys[], uint32_t u32KeySize)
{
    ...
    key_ptr = (uint32_t *)((uint32_t)&CRPT->AES0_KEY[0] + (u32Channel * 0x3C));
    wcnt = 4 + u32KeySize*2;
...
void AES_SetInitVect(uint32_t u32Channel, uint32_t au32IV[])
{
    ...
    key_ptr = (uint32_t *)((uint32_t)&CRPT->AES0_IV[0] + (u32Channel * 0x3C));
...
void SHA_Read(uint32_t u32Digest[])
{
    ...
    for (i = 0; i < wcnt; i++)
        u32Digest[i] = *(uint32_t *)((uint32_t)&(CRPT->HMAC_DGST[0]) + (i * 4));
}
...
void ECC_DriverISR()
{
    if (CRPT->INTSTS & CRPT_INTSTS_ECCIF_Msk) {
        ...
        CRPT_DBGMSG("ECC done IRQ.\n");
    }

    if (CRPT->INTSTS & CRPT_INTSTS_ECCEIF_Msk) {
        g_ECCERR_done = 1;
        CRPT->INTSTS = CRPT_INTSTS_ECCEIF_Msk;
        CRPT_DBGMSG("ECCERRIF is set!!\n");
    }
}

#if 0 //ENABLE_DEBUG
static void dump_ecc_reg(char *str, uint32_t *regs, int count)
{
    int  i;
    printf("%s => ", str);
    for (i = 0; i < count; i++)
...
static void Reg2Hex(int count, uint32_t *reg, char *output)
{
    int  idx, i, ri, n;
...
static ECC_CURVE * get_curve(E_ECC_CURVE ecc_curve)
{
    ...
    for (i = 0; i < sizeof(_Curve) / sizeof(ECC_CURVE); i++) {
        if (ecc_curve == _Curve[i].curve_id) {
            memcpy((char *)&Curve_Copy, (char *)&_Curve[i], sizeof(ECC_CURVE));
            return &Curve_Copy; //(ECC_CURVE *)&_Curve[i];
        }
    }
    return NULL;
}

static int ecc_init_curve(E_ECC_CURVE ecc_curve)
{
    int  i;
    ...
    Hex2Reg(pCurve->Ea, (uint32_t *)&CRPT->ECC_A[0]);
    Hex2Reg(pCurve->Eb, (uint32_t *)&CRPT->ECC_B[0]);
    Hex2Reg(pCurve->Px, (uint32_t *)&CRPT->ECC_X1[0]);
    Hex2Reg(pCurve->Py, (uint32_t *)&CRPT->ECC_Y1[0]);

    //CRPT_DBGMSG("Key length = %d\n", pCurve->key_len);
    //dump_ecc_reg("CRPT_ECC_CURVE_A", (uint32_t *)&CRPT->ECC_A[0], 10);
    //dump_ecc_reg("CRPT_ECC_CURVE_B", (uint32_t *)&CRPT->ECC_B[0], 10);
    //dump_ecc_reg("CRPT_ECC_POINT_X1", (uint32_t *)&CRPT->ECC_X1[0], 10);
    //dump_ecc_reg("CRPT_ECC_POINT_Y1", (uint32_t *)&CRPT->ECC_Y1[0], 10);

    if (pCurve->GF == CURVE_GF_2M) {
        CRPT->ECC_N[0] = 0x1;
        CRPT->ECC_N[(pCurve->key_len) / 32] |= (1 << ((pCurve->key_len) % 32));
        CRPT->ECC_N[(pCurve->irreducible_k1) / 32] |= (1 << ((pCurve->irreducible_k1) % 32));
        CRPT->ECC_N[(pCurve->irreducible_k2) / 32] |= (1 << ((pCurve->irreducible_k2) % 32));
        CRPT->ECC_N[(pCurve->irreducible_k3) / 32] |= (1 << ((pCurve->irreducible_k3) % 32));
    } else {
        Hex2Reg(pCurve->Pp, (uint32_t *)&CRPT->ECC_N[0]);
    }

    //dump_ecc_reg("CRPT_ECC_CURVE_N", (uint32_t *)&CRPT->ECC_N[0], 10);
...
int  ECC_GeneratePublicKey(E_ECC_CURVE ecc_curve, char *private_k, char *public_k1, char *public_k2)
{
    ...
    Hex2Reg(private_k, (uint32_t *)&CRPT->ECC_K[0]);
    ...
    g_ECC_done = g_ECCERR_done = 0;
    CRPT->ECC_CTL |= (pCurve->key_len << CRPT_ECC_CTL_CURVEM_Pos) | ECCOP_POINT_MUL | CRPT_ECC_CTL_START_Msk;
    while ((g_ECC_done == 0) && (g_ECCERR_done == 0));

    Reg2Hex(pCurve->Echar, (uint32_t *)&CRPT->ECC_X1[0], public_k1);
    Reg2Hex(pCurve->Echar, (uint32_t *)&CRPT->ECC_Y1[0], public_k2);
...
static void run_ecc_codec(uint32_t mode)
{
    ...
    g_ECC_done = g_ECCERR_done = 0;
    CRPT->ECC_CTL |= (pCurve->key_len << CRPT_ECC_CTL_CURVEM_Pos) | mode | CRPT_ECC_CTL_START_Msk;
    while ((g_ECC_done == 0) && (g_ECCERR_done == 0));
...
/**
  ...
  * @return  -1   "ecc_curve" value is invalid.
  */
int  ECC_GenerateSignature(E_ECC_CURVE ecc_curve, char *message,
                           char *d, char *k, char *R, char *S)
{
    ...
    Hex2Reg(k, (uint32_t *)&CRPT->ECC_K[0]);
    ...
    for (i = 0; i < 18; i++)
        CRPT->ECC_N[i] = 0;
    Hex2Reg(pCurve->Eorder, (uint32_t *)&CRPT->ECC_N[0]);
    ...
    Hex2Reg(k, (uint32_t *)&CRPT->ECC_X1[0]);

    run_ecc_codec(ECCOP_MODULE | MODOP_DIV);

#if ENABLE_DEBUG
    Reg2Hex(pCurve->Echar, (uint32_t *)&CRPT->ECC_X1[0], temp_hex_str);
    ...
    for (i = 0; i < 18; i++)
        CRPT->ECC_Y1[i] = 0;
    Hex2Reg(d, (uint32_t *)&CRPT->ECC_Y1[0]);
    ...
    Hex2Reg(message, (uint32_t *)&CRPT->ECC_Y1[0]);
                         ...
                         char *public_k1, char *public_k2, char *R, char *S)
{
    ...
    Hex2Reg(S, (uint32_t *)&CRPT->ECC_X1[0]);
    ...
    for (i = 0; i < 18; i++)
        CRPT->ECC_X1[i] = 0;
    Hex2Reg(message, (uint32_t *)&CRPT->ECC_X1[0]);
    for (i = 0; i < 18; i++)
        CRPT->ECC_Y1[i] = temp_result2[i];
    ...
    for (i = 0; i < 18; i++)
        CRPT->ECC_X1[i] = 0;
    Hex2Reg(R, (uint32_t *)&CRPT->ECC_X1[0]);
    ...
    Hex2Reg(public_k1, (uint32_t *)&CRPT->ECC_X1[0]);
    Hex2Reg(public_k2, (uint32_t *)&CRPT->ECC_Y1[0]);
    ...
    Reg2Hex(pCurve->Echar, (uint32_t *)&CRPT->ECC_Y1[0], temp_hex_str);
