 ...
 * @brief    M480 series PWM driver source file
 *
 * @note
...
uint32_t PWM_ConfigCaptureChannel(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32UnitTimeNsec, uint32_t u32CaptureEdge)
{
    ...
    (pwm)->CTL1 = ((pwm)->CTL1 & ~((1 << PWM_CTL1_CNTTYPEn_Pos) << (u32ChannelNum << 1))) | (1UL << (u32ChannelNum << 1));
    // set PWM to auto-reload mode
    (pwm)->CTL1 &= ~((1 << PWM_CTL1_CNTMODEn_Pos) << u32ChannelNum);
                                 ...
                                 uint32_t u32Frequency2)
{
    ...
    (pwm)->CTL1 = ((pwm)->CTL1 & ~(((1 << PWM_CTL1_CNTTYPEn_Pos) << (u32ChannelNum << 1))|((1 << PWM_CTL1_CNTMODEn_Pos) << u32ChannelNum)));

    PWM_SET_CNR(pwm, u32ChannelNum, --u16CNR);
    PWM_SET_CMR(pwm, u32ChannelNum, u32DutyCycle * (u16CNR + 1) / 100);

    (pwm)->WGCTL0 = ((pwm)->WGCTL0 & ~(((1 << PWM_WGCTL0_PRDPCTLn_Pos) | (1 << PWM_WGCTL0_ZPCTLn_Pos)) << (u32ChannelNum << 1))) | \
                    (PWM_OUTPUT_HIGH << ((u32ChannelNum << 1) + PWM_WGCTL0_ZPCTLn_Pos));
    (pwm)->WGCTL1 = ((pwm)->WGCTL1 & ~(((1 << PWM_WGCTL1_CMPDCTLn_Pos) | (1 << PWM_WGCTL1_CMPUCTLn_Pos)) << (u32ChannelNum << 1))) | \
                    (PWM_OUTPUT_LOW << ((u32ChannelNum << 1) + PWM_WGCTL1_CMPUCTLn_Pos));
...
{
    (pwm)->STATUS = (PWM_STATUS_ADCTRGFn_Msk << u32ChannelNum);
}
...
{
    return (((pwm)->STATUS & (PWM_STATUS_ADCTRGFn_Msk << u32ChannelNum)) ? 1 : 0);
}
...
{
    return (((((pwm)->CAPIF & (PWM_CAPIF_CFLIFn_Msk << u32ChannelNum)) ? 1 : 0) << 1) | \
            (((pwm)->CAPIF & (PWM_CAPIF_CRLIFn_Msk << u32ChannelNum)) ? 1 : 0));
}
/**
...
{
    (pwm)->INTSTS0 = (PWM_INTSTS0_CMPUIFn_Msk | PWM_INTSTS0_CMPDIFn_Msk) << u32ChannelNum;
}
...
{
    return ((((pwm)->INTSTS0 & ((PWM_INTSTS0_CMPDIFn_Msk | PWM_INTSTS0_CMPUIFn_Msk) << u32ChannelNum))) ? 1 : 0);
}
...
{
    (pwm)->INTEN0 |= ((1 << PWM_INTEN0_PIENn_Pos) << u32ChannelNum);
}
...
{
    (pwm)->INTSTS0 = ((1 << PWM_INTSTS0_PIFn_Pos) << u32ChannelNum);
}
...
{
    return ((((pwm)->INTSTS0 & ((1 << PWM_INTSTS0_PIFn_Pos) << u32ChannelNum))) ? 1 : 0);
}
...
{
    (pwm)->INTEN0 |= ((1 << PWM_INTEN0_ZIENn_Pos) << u32ChannelNum);
}
...
{
    (pwm)->INTSTS0 = ((1 << PWM_INTEN0_ZIENn_Pos) << u32ChannelNum);
}
...
{
    return ((((pwm)->INTSTS0 & ((1 << PWM_INTEN0_ZIENn_Pos) << u32ChannelNum))) ? 1 : 0);
}
...
{
    (pwm)->FTCI = ((PWM_FTCI_FTCMUn_Msk | PWM_FTCI_FTCMDn_Msk) << (u32ChannelNum >> 1));
}
...
{
    return (((pwm)->FTCI & ((PWM_FTCI_FTCMUn_Msk | PWM_FTCI_FTCMDn_Msk) << (u32ChannelNum >> 1))) ? 1 : 0);
}
...
void PWM_ConfigSyncPhase(PWM_T *pwm, uint32_t u32ChannelNum, uint32_t u32SyncSrc, uint32_t u32Direction, uint32_t u32StartPhase)
{
    // every two channels shares the same setting
    u32ChannelNum >>= 1;
    (pwm)->SYNC = (((pwm)->SYNC & ~(((3 << PWM_SYNC_SINSRCn_Pos) << (u32ChannelNum << 1)) | ((1 << PWM_SYNC_PHSDIRn_Pos) << u32ChannelNum))) | \
                   (u32Direction << PWM_SYNC_PHSDIRn_Pos << u32ChannelNum) | ((u32SyncSrc << PWM_SYNC_SINSRCn_Pos) << (u32ChannelNum << 1)));
...
void PWM_EnableSyncPhase(PWM_T *pwm, uint32_t u32ChannelMask)
{
    ...
    for(i = 0; i < PWM_CHANNEL_NUM; i ++) {
        if(u32ChannelMask & (1 << i)) {
            (pwm)->SYNC |= ((1 << PWM_SYNC_PHSENn_Pos) << (i >> 1));
        }
    }
}
...
{
    return (((pwm)->STATUS & (PWM_STATUS_CNTMAXFn_Msk << u32ChannelNum)) ? 1 : 0);
}
...
{
    (pwm)->STATUS = (PWM_STATUS_CNTMAXFn_Msk << u32ChannelNum);
}
