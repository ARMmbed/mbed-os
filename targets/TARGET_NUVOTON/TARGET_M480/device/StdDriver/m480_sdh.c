...
{
    uint32_t rate, div1;
    static uint32_t u32SD_ClkSrc = 0, u32SD_PwrCtl = 0;
    ...
    if (sd_clock_khz <= 400)
    {
        ...
        if ((u32SD_PwrCtl & CLK_PWRCTL_HIRCEN_Msk) != 0x4)
...
void SDH_CardSelect(SDH_T *sdh, int cardSel)
{
    if(cardSel == 0) {
        sdh->CTL |= (sdh->CTL & ~SDH_CTL_SDPORT_Msk);
    } else if(cardSel == 1) {
        sdh->CTL |= ((sdh->CTL & ~SDH_CTL_SDPORT_Msk) | (1 << SDH_CTL_SDPORT_Pos));
    }
}
...
void SDH_Open(SDH_T *sdh, uint32_t u32CardDetSrc)
{
    ...
    if(u32CardDetSrc & CardDetect_From_DAT3) {
        printf("CardDetect_From_DAT3\n");
        sdh->INTEN &= ~SDH_INTEN_CDSRC0_Msk;
    } else {
        printf("CardDetect_From_GPIO\n");
        sdh->INTEN |= SDH_INTEN_CDSRC0_Msk;
    }
    ...
    sdh->CTL &= ~((0xFF) | (SDH_CTL_CLKKEEP1_Msk));    // disable SD clock output
}
...
uint32_t SDH_Probe(SDH_T *sdh)
{
    ...
    sdh->CTL |=  0x09 << SDH_CTL_SDNWR_Pos;         // set SDNWR = 9
    sdh->CTL &= ~SDH_CTL_BLKCNT_Msk;
    sdh->CTL |=  0x01 << SDH_CTL_BLKCNT_Pos;           // set BLKCNT = 1
    sdh->CTL &= ~SDH_CTL_DBW_Msk;               // SD 1-bit data bus

    if(!(SDH_CardDetection(sdh)))
        return SDH_NO_SD_CARD;

    if ((val = SDH_Init(sdh)) != 0)
        return val;
    ...
    if ((val = SDH_SelectCardType(sdh)) != 0)
        return val;
...
uint32_t SDH_Read(SDH_T *sdh, uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    ...
    sdh->BLEN = blksize - 1;       // the actual byte count is equal to (SDBLEN+1)
    ...
    for (i=0; i<loop; i++) {
        ...
        if (bIsSendCmd == FALSE) {
            sdh->CTL = reg|(18<<8)|(SDH_CTL_COEN_Msk | SDH_CTL_RIEN_Msk | SDH_CTL_DIEN_Msk);
            bIsSendCmd = TRUE;
        } else
...
uint32_t SDH_Write(SDH_T *sdh, uint8_t *pu8BufAddr, uint32_t u32StartSec, uint32_t u32SecCount)
{
    ...
    for (i=0; i<loop; i++) {
        ...
        if (!bIsSendCmd) {
            sdh->CTL = reg|(25<<8)|(SDH_CTL_COEN_Msk | SDH_CTL_RIEN_Msk | SDH_CTL_DOEN_Msk);
            bIsSendCmd = TRUE;
        } else
        ...
        if ((sdh->INTSTS & SDH_INTSTS_CRCIF_Msk) != 0) {   // check CRC
            sdh->INTSTS = SDH_INTSTS_CRCIF_Msk;
