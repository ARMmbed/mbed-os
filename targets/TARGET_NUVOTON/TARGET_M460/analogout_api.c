...
{   
    ...
    MBED_ASSERT(modinit->modname == (int) obj->dac);
...
{   
    DAC_T *dac_base = (DAC_T *) NU_MODBASE(obj->dac);
    /* Suppress 'unused variable' warning in case of macro function below expanding to none */
    MBED_UNUSED uint32_t chn = NU_MODSUBINDEX(obj->dac);
