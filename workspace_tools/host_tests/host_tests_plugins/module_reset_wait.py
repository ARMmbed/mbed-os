...
class HostTestPluginResetMethod_Wait(HostTestPluginBase):
    ...
    # Plugin interface
    name = 'HostTestPluginResetMethod_Wait'
    type = 'ResetMethod'
    stable = True
    capabilities = ['wait']
    ...
    def execute(self, capabilitity, *args, **kwargs):
        ...
        if self.check_parameters(capabilitity, *args, **kwargs) is True:
            if capabilitity == 'wait':
                serial = kwargs['serial']
                 
                # Wait for a magical 2 seconds (Helps with boards that reset after flashing)
                print('HOST: Sleeping for 2 seconds')
                sleep(2)

                # Flush the serial buffer (again, for boards that reset after flashing)
                print('HOST: Flushing serial buffer')
                serial.flushInput()
                serial.flushOutput()
...
def load_plugin():
    ...
    """
    return HostTestPluginResetMethod_Wait()
