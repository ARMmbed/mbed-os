...
int thread_management_security_material_set(int8_t nwk_interface_id, bool enableSecurity, uint8_t *thrMasterKey, uint32_t thrKeySequenceCounter, uint32_t thrKeyRotation)
{
    return 0;
}
...
int thread_management_joiner_attach_start(int8_t nwk_interface_id)
{
    return 0;
}

uint8_t *thread_management_key_request(int8_t nwk_interface_id, uint8_t keyId, bool linkLayerKey)
{
    return NULL;
}

uint8_t *thread_management_key_request_with_sequence(int8_t nwk_interface_id, uint8_t keyId, uint32_t keySequnce)
{
    return NULL;
}

void thread_management_key_synch_req(int8_t nwk_interface_id, uint32_t keySequnce, bool acceptAnyCounter)
{
}

void thread_history_key_material_push(thread_info_t *thread_info, uint8_t *macKeyPtr, uint8_t *mleKeyPtr, uint8_t keyId)
{
}
...
int thread_management_increment_key_sequence_counter(int8_t nwk_interface_id)
{
    return 0;
}
...
int thread_management_if_beacon_payload_generate(protocol_interface_info_entry_t *cur, const link_configuration_s *linkConfiguration)
{
    return 0;
}

int thread_management_network_data_tlv_push(protocol_interface_info_entry_t *cur, uint8_t *data_pointer, uint8_t length)
{
    return 0;
}
...
int thread_management_network_data_register(int8_t interface_id, uint8_t *network_data_ptr, uint8_t *network_data_length, uint16_t routerId)
{
    return 0;
}

int thread_management_publish_local_services(int8_t interface_id)
{
    return 0;
}

int thread_management_delete_local_services(int8_t interface_id)
{
    return 0;
}
