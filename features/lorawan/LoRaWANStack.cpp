...
//#define LORAWAN_COMPLIANCE_TEST_DUTYCYCLE_ON                          true
...
bool LoRaWANStack::is_port_valid(uint8_t port)
{
    ...
    if (port >= 224 || port == 0) {
        // Pass port check if compliance test is running.
        if (port == 224 && _compliance_test.running == true) {
            return true;
        }
        return false;
    }
...
lora_mac_status_t LoRaWANStack::initialize_mac_layer()
{
    ...
    static uint8_t compliance_test_buffer[LORAMAC_PHY_MAXPAYLOAD];
 ...
 * \return                  The length of data sent or a negative
 *                          error code.
 */
...
lora_mac_status_t LoRaWANStack::send_frame_to_mac()
{
   ...
   // LoRaMacTxInfo_t txInfo;
    ...
    if (LORA_MCPS_UNCONFIRMED == mcps_req.type) {
        ...
        mcps_req.f_buffer = _tx_msg.f_buffer;
        mcps_req.f_buffer_size = _tx_msg.f_buffer_size;
        mcps_req.req.confirmed.nb_trials =
                _tx_msg.message_u.confirmed.nb_trials;
                                ...
                                uint16_t length, uint8_t flags)
{
    ...
    if (port == 224 && _compliance_test.running == true) {
        // Special handling for compliance test.
        _tx_msg.f_buffer_size = prepare_special_tx_frame(port);
    } else if (_compliance_test.running == false) {
        ...
        tr_info("RTS = %d bytes, PEND = %d", _tx_msg.f_buffer_size, _tx_msg.pending_size);
    }

    tr_info("RTS = %u bytes, PEND = %u", _tx_msg.f_buffer_size, _tx_msg.pending_size);

    // If duty cycling is on, user defined timers are not in use and
    // hence we need to tell the state machine that now is the time to schedule
    // to transmission. Stack will decide when to send in actuality based
    // upon duty cycling calculations
    if (_duty_cycle_on) {
        tr_debug("Duty Cycle - ON");

        set_device_state(DEVICE_STATE_SEND);
        lora_state_machine();
    } else {
        tr_debug("Duty Cycle - OFF");
        // Use user defined timers for uplink
        if(_compliance_test.running == true) {
            tx_duty_cycle_time = 5000; //5000ms
        } else {
            tx_duty_cycle_time = TX_TIMER +
                    ((int32_t)rand( ) % ( TX_TIMER_RND - (-TX_TIMER_RND) + 1 ) + (-TX_TIMER_RND));
        }

        status = error_type_converter(LoRaMacSetTxTimer(tx_duty_cycle_time));
        if (status != LORA_MAC_STATUS_OK) {
            return status;
        }
    }
        ...
        case 224:
            if (_compliance_test.running == false) {
                ...
                case 4: // (vii)
                    ...
                    handle_tx(224, _compliance_test.app_data_buffer,
                            _compliance_test.app_data_size,
                            _compliance_test.is_tx_confirmed == true ? MSG_CONFIRMED_FLAG : MSG_UNCONFIRMED_FLAG);
                    break;
                case 5: // (viii)
                ...
                case 6: // (ix)
                    ...
                    if (_lw_session.connection.connect_type == LORAWAN_CONNECTION_OTAA) {
                        ...
                        // Normal operation was connected by ABP.
            ...
            break;
        default:
            if ((is_port_valid(mcps_indication->port) == true && _compliance_test.running == false)
                    || (mcps_indication->mcps_indication == LORA_MCPS_PROPRIETARY && _compliance_test.running == false)) {
    ...
    switch (_device_current_state) {
        ...
        case DEVICE_STATE_SEND:
            ...
            if (_tx_msg.tx_ongoing) {
                ...
                // Special handling if compliance test is executing..
                if (_compliance_test.running == true) {
                    handle_tx(224, _compliance_test.app_data_buffer,
                            _compliance_test.app_data_size,
                            _compliance_test.is_tx_confirmed == true ? MSG_CONFIRMED_FLAG : MSG_UNCONFIRMED_FLAG);
