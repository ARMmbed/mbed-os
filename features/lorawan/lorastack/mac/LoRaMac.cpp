 ...
 * Signals
 */
#define SIG_TXDONE             0x01
#define SIG_RXDONE             0x02
#define SIG_CADDONE            0x04
#define SIG_RXERROR            0x08
#define SIG_TXTIMEOUT          0x10
#define SIG_RXTIMEOUT          0x20
#define SIG_FHSS_CC            0x40
#define SIG_RXWIN1             0x80
#define SIG_RXWIN2             0x100
#define SIG_ACK_TIMEOUT        0x200
#define SIG_DELAYED_TX         0x400
#define SIG_MAC_STAT_CHK       0x800
#define SIG_TX_NEXT            0x1000

/**
 * We need to suppress fast  signaling as
 * Queue for deferred event signals is only 16
 * items long. This will prevent ISR Queue overflow.
 */
...
/**
 * Structure to delegate parameters of radio events
 *
 * Radio driver is passed radio_events data structure
 * that contains callbacks implemented in LoRaMAC.cpp.
 * Those callbacks need to be deferred in multithreaded
 * environment. This structure temporarily stores the
 * parameter of those callbacks, for being used in deferred
 * calls.
 */
typedef struct {
    bool cad;
    ...
    int8_t snr;
    uint8_t cur_channel;
} radio_event_parameters;

static radio_event_parameters cb_params;
...
public:
    ...
    void lock();
    void unlock();
    rtos::Thread mac_process_thread;
    rtos::Mutex mutex;
};

LoRaMACTask::LoRaMACTask() : mac_process_thread(osPriorityNormal, 2048)
{
    mac_process_thread.start(mbed::callback(this, &LoRaMACTask::process_task));
}

void LoRaMACTask::lock()
{
    mutex.lock();
}

void LoRaMACTask::unlock()
{
    mutex.unlock();
}

void LoRaMACTask::process_task(void)
{
    for (;;) {
        lock();
        osEvent event = mac_process_thread.signal_wait(0, osWaitForever);
        ...
        if (event.value.signals & SIG_TXDONE) {
            OnRadioTxDone();
        }
        if (event.value.signals & SIG_RXDONE) {
           OnRadioRxDone(cb_params.payload, cb_params.size,
                                   cb_params.rssi, cb_params.snr);
        }
        if (event.value.signals & SIG_RXERROR) {
            OnRadioRxError();
        }
        if (event.value.signals & SIG_TXTIMEOUT) {
            OnRadioTxTimeout();
        }
        if (event.value.signals & SIG_RXTIMEOUT) {
            OnRadioRxTimeout();
        }
        if (event.value.signals & SIG_CADDONE) {
            //TODO not implemented yet
            //OnRadioCadDone(cb_params.cad);
        }
        if (event.value.signals & SIG_FHSS_CC) {
            //TODO not implemented yet
            //OnFHSSCHannelChange(cb_params.cur_channel);
        }
        if (event.value.signals & SIG_RXWIN1) {
            OnRxWindow1TimerEvent();
        }
        if (event.value.signals & SIG_RXWIN2) {
            OnRxWindow2TimerEvent();
        }
        if (event.value.signals & SIG_ACK_TIMEOUT) {
            OnAckTimeoutTimerEvent();
        }
        if (event.value.signals & SIG_DELAYED_TX) {
            OnTxDelayedTimerEvent();
        }
        if (event.value.signals & SIG_MAC_STAT_CHK) {
            OnMacStateCheckTimerEvent();
        }
        if (event.value.signals & SIG_TX_NEXT) {
            OnNextTx();
        }
        unlock();
    }
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_TXDONE);
        signaled = true;
    }
#else
    OnRadioTxDone();
...
{
#ifdef MBED_CONF_RTOS_PRESENT
    cb_params.payload = payload;
    cb_params.size = size;
    cb_params.rssi = rssi;
    cb_params.snr = snr;
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_RXDONE);
        signaled = true;
    }
#else
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_RXERROR);
        signaled = true;
    }
#else
    OnRadioRxError();
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_RXTIMEOUT);
        signaled = true;
    }
#else
    OnRadioRxTimeout();
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_TXTIMEOUT);
        signaled = true;
    }
#else
   OnRadioTxTimeout();
...
static void handle_cad_done(bool cad)
{
    ...
    cb_params.cad = cad;
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_CADDONE);
        signaled = true;
    }
#else
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_FHSS_CC);
        signaled = true;
    }
    cb_params.cur_channel = cur_channel;
#else
    // TODO Not implemented yet
    //OnRadioFHSSChangeChannelcur_channel();
#endif
}
static void handle_mac_state_check_timer_event(void)
{
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_MAC_STAT_CHK);
        signaled = true;
    }
#else
    OnMacStateCheckTimerEvent();
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_TX_NEXT);
        signaled = true;
    }
#else
    OnNextTx();
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_DELAYED_TX);
        signaled = true;
    }
#else
    OnTxDelayedTimerEvent();
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_ACK_TIMEOUT);
        signaled = true;
    }
#else
    OnAckTimeoutTimerEvent();
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_RXWIN1);
        signaled = true;
    }
#else
    OnRxWindow1TimerEvent();
...
#ifdef MBED_CONF_RTOS_PRESENT
    if (!signaled) {
        mac_task.mac_process_thread.signal_set(SIG_RXWIN2);
        signaled = true;
    }
#else
    OnRxWindow2TimerEvent();
