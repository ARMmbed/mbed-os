 ...
 */
static void sl_eth_add_mcast(uint8_t* addr);

/**
 * Remove previously added MAC address from address filter
 */
static void sl_eth_del_mcast(uint8_t* addr);
...
static sl_eth_rx_bd_t     sl_eth_rx_bds[SL_ETH_NUM_RX_BD];
static struct pbuf *      sl_eth_rx_pbufs[SL_ETH_NUM_RX_BD];
static sl_eth_tx_bd_t     sl_eth_tx_bds[SL_ETH_NUM_TX_BD];
...
static void sl_eth_data_init(void) {
  ...
  for(i = 0; i < SL_ETH_NUM_RX_BD; i++) {
    ...
    sl_eth_rx_bds[i].rx_addr = ((uint32_t)sl_eth_rx_pbufs[i]->payload) & ~0x3;
    if(i == SL_ETH_NUM_RX_BD-1) {
      sl_eth_rx_bds[i].rx_addr |= 0x2;
    }
...
/** \brief  Signals one or more packets have been sent, and we can release the
...
static void sl_eth_rx_done_cb(struct netif *netif) {
  ...
  while(sl_eth_rx_bds[state->rx_idx].rx_addr & 0x1) {
    ...
    if (NULL == temp_rxbuf) {
      ...
      state->rx_idx));
      sl_eth_rx_bds[state->rx_idx].rx_addr &= ~0x1;
    } else {
      sl_eth_rx_pbufs[state->rx_idx] = temp_rxbuf;

      sl_eth_rx_bds[state->rx_idx].status = 0;
      if(sl_eth_rx_bds[state->rx_idx].rx_addr & 0x2) {
        sl_eth_rx_bds[state->rx_idx].rx_addr = (uint32_t)temp_rxbuf->payload | 0x2;
      } else {
        sl_eth_rx_bds[state->rx_idx].rx_addr = (uint32_t)temp_rxbuf->payload;
      }
 ...
 * \return ERR_OK or error code
 */
err_t sl_eth_igmp_filter(struct netif *netif,
 ...
 * \return ERR_OK or error code
 */
err_t sl_eth_mld_filter(struct netif *netif,
...
static void sl_eth_add_mcast(uint8_t* addr) {
  ...
  bitnr = ((addr[0]       & 0x3F)                              & 0x3F)
          ^ ((((addr[0] >> 6) & 0x3) | ((addr[1] & 0xF) << 2)) & 0x3F)
          ^ ((((addr[1] >> 4) & 0xF) | ((addr[2] & 0x3) << 4)) & 0x3F)
          ^ ((((addr[2] >> 2) & 0x3F))                         & 0x3F)
          ^   ((addr[3]       & 0x3F)                          & 0x3F)
          ^ ((((addr[3] >> 6) & 0x3) | ((addr[4] & 0xF) << 2)) & 0x3F)
          ^ ((((addr[4] >> 4) & 0xF) | ((addr[5] & 0x3) << 4)) & 0x3F)
          ^ ((((addr[5] >> 2) & 0x3F))                         & 0x3F);
  /* Increment refcnt */
