...
static err_t igmp_mac_filter(struct netif *netif, const ip4_addr_t *group, u8_t action)
{
    ...
    switch (action) {
        case NETIF_ADD_MAC_FILTER:
        {
            ...
            addr[4] = group23 >> 8;
            addr[5] = group23;
            emac->ops->add_multicast_group(emac->hw, addr);
            return ERR_OK;
        }
...
err_t emac_lwip_if_init(struct netif *netif)
{
    ...
    netif->hwaddr_len = emac->ops->get_hwaddr_size(emac->hw);
    if (emac->ops->get_hwaddr) {
        emac->ops->get_hwaddr(emac->hw, netif->hwaddr);
    }
    emac->ops->set_hwaddr(emac->hw, netif->hwaddr);
