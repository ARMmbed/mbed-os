 ...
 *  match the device configuration up to a CRC before locking. Hence it
 *  protects against most accidental errors.
 *
 *  Note this data is deliberately not constant. First 4 blocks of 4 bytes
 *  contain device specific data that changes from device to device. It is
 *  replaced by reading these blocks from the device before calculating the
 *  CRC.
 */
