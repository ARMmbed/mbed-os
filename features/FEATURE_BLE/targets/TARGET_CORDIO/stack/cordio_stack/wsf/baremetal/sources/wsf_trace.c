...
struct
{
  ...
  WsfBt4TraceCback_t bt4cback;          /*!< Platform trace callback for BT4. */
...
void WsfTrace(const char *pStr, ...)
{
  ...
  if (wsfTraceCb.bt4cback != NULL)
  {
    va_list args;

    va_start(args, pStr);
    if (wsfTraceCb.bt4cback)
    {
      (*wsfTraceCb.bt4cback)(pStr, args);
    }
    va_end(args);
  }
  else if (wsfTraceCb.sendMsgCback != NULL)
  {
    uint32_t len;
    va_list  args;
