...
struct read_control_block_t : public procedure_control_block_t {
	...
	uint16_t get_mtu_size(GenericGattClient* client) {
		uint16_t result = 23;
		ble_error_t err = client->_pal_client->get_mtu_size(connection_handle, result);
		if (err) {
			result = 23;
		}
	...
	void handle_read_response(GenericGattClient* client, const ResponseType& read_response) {
		uint16_t mtu_size = get_mtu_size(client);
