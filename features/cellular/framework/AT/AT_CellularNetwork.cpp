...
AT_CellularNetwork::AT_CellularNetwork(ATHandler &atHandler, CellularDevice& device) :
    ...
    _at(atHandler), _device(device)
{
    ...
    for (int type = 0; type < CellularNetwork::C_MAX; type++) {
        if (_device.get_property((CellularDevice::CellularProperty)type) != RegistrationModeDisable) {
            _at.set_urc_handler(at_reg[type].urc_prefix, _urc_funcs[type]);
        }
    }

    if (_device.get_property(CellularDevice::PROPERTY_AT_CGEREP)) {
...
nsapi_error_t AT_CellularNetwork::set_registration_urc(RegistrationType type, bool urc_on)
{
    ...
    RegistrationMode mode = (RegistrationMode)_device.get_property((CellularDevice::CellularProperty)type);
...
nsapi_error_t AT_CellularNetwork::get_registration_params(RegistrationType type, registration_params_t &reg_params)
{
    ...
    if (!_device.get_property((CellularDevice::CellularProperty)at_reg[i].type)) {
        return NSAPI_ERROR_UNSUPPORTED;
    }
...
nsapi_error_t AT_CellularNetwork::clear()
{
    ...
    if (contexts.get_head()) {
        ...
        if (_device.get_property(CellularDevice::PROPERTY_IPV4V6_PDP_TYPE) ||
                (_device.get_property(CellularDevice::PROPERTY_IPV4_PDP_TYPE) && _device.get_property(CellularDevice::PROPERTY_IPV6_PDP_TYPE))) {
            strcpy(pdp_type_str, "IPV4V6");
        } else if (_device.get_property(CellularDevice::PROPERTY_IPV6_PDP_TYPE)) {
