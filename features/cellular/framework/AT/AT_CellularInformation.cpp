...
AT_CellularInformation::AT_CellularInformation(ATHandler &at, CellularDevice& device) : _at(at), _device(device)
{
}
...
nsapi_error_t AT_CellularInformation::get_serial_number(char *buf, size_t buf_size, SerialNumberType type)
{
    ...
    if (!_device.get_property(CellularDevice::PROPERTY_AT_CGSN_WITH_TYPE)) {
