...
nsapi_error_t AT_CellularContext::do_user_authentication()
{
    ...
    if (_pwd && _uname) {
        if (!get_device()->get_property(CellularDevice::PROPERTY_AT_CGAUTH)) {
            return NSAPI_ERROR_UNSUPPORTED;
        }
...
CellularDevice::CellularProperty AT_CellularContext::pdp_type_t_to_cellular_property(pdp_type_t pdp_type)
{
    CellularDevice::CellularProperty prop = CellularDevice::PROPERTY_IPV4_PDP_TYPE;
    if (pdp_type == IPV6_PDP_TYPE) {
        prop = CellularDevice::PROPERTY_IPV6_PDP_TYPE;
    } else if (pdp_type == IPV4V6_PDP_TYPE) {
        prop = CellularDevice::PROPERTY_IPV4V6_PDP_TYPE;
    } else if (pdp_type == NON_IP_PDP_TYPE) {
        prop = CellularDevice::PROPERTY_NON_IP_PDP_TYPE;
    }
        ...
        if (pdp_type_len > 0) {
            ...
            if (apn_len >= 0) {
                ...
                if (get_device()->get_property(pdp_type_t_to_cellular_property(pdp_type)) ||
                        ((pdp_type == IPV4V6_PDP_TYPE && (get_device()->get_property(CellularDevice::PROPERTY_IPV4_PDP_TYPE) &&
                                                          get_device()->get_property(CellularDevice::PROPERTY_IPV6_PDP_TYPE))) && !_nonip_req)) {
                    _pdp_type = pdp_type;
                    set_cid(cid);
                }
            }
...
bool AT_CellularContext::set_new_context(int cid)
{
    ...
    if (_nonip_req && _cp_in_use && get_device()->get_property(CellularDevice::PROPERTY_NON_IP_PDP_TYPE)) {
        strncpy(pdp_type_str, "Non-IP", sizeof(pdp_type_str));
        pdp_type = NON_IP_PDP_TYPE;
    } else if (get_device()->get_property(CellularDevice::PROPERTY_IPV4V6_PDP_TYPE) ||
               (get_device()->get_property(CellularDevice::PROPERTY_IPV4_PDP_TYPE) &&
                get_device()->get_property(CellularDevice::PROPERTY_IPV6_PDP_TYPE))) {
        strncpy(pdp_type_str, "IPV4V6", sizeof(pdp_type_str));
        pdp_type = IPV4V6_PDP_TYPE;
    } else if (get_device()->get_property(CellularDevice::PROPERTY_IPV6_PDP_TYPE)) {
        strncpy(pdp_type_str, "IPV6", sizeof(pdp_type_str));
        pdp_type = IPV6_PDP_TYPE;
    } else if (get_device()->get_property(CellularDevice::PROPERTY_IPV4_PDP_TYPE)) {
...
nsapi_error_t AT_CellularContext::open_data_channel()
{
    ...
    if (get_device()->get_property(CellularDevice::PROPERTY_AT_CGDATA)) {
