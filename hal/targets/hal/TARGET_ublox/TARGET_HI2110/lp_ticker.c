...
/* The lower 32 bits of the compare-match value */
static uint32_t g_reportable_compare_value;
...
/* Flag to ensure we only wake the system up from sleep on
 * user interrupt events */
static volatile bool g_wake_up_now = false;

/* ----------------------------------------------------------------
 * FUNCTION PROTOTYPES
 * ----------------------------------------------------------------*/

static void set_interrupt_to_32bit_overflow();
static void set_interrupt_to_user_value();
...
static inline uint64_t get_rtc_value()
{
    ...
    rtcValue = ((uint64_t) RTC_TIME_MSBS) << 32;
    rtcValue |= RTC_TIME_LSBS;
...
void lp_ticker_set_interrupt(timestamp_t time)
{
    ...
    /* mbed needs to be able to read back the value it has set but, again,
     * in 32-bit modulo terms.  So the value it needs is relative to the
     * last overflow value */
    g_reportable_compare_value = (uint32_t) (g_next_compare_value - g_last_32bit_overflow_value);
