...
#if defined(MCU_MEM_MAP_VERSION) && ((MCU_MEM_MAP_VERSION > 2) || ((MCU_MEM_MAP_VERSION == 2) && (MCU_MEM_MAP_VERSION_MINOR >= 8)))
...
static inline void setupNextProgramData(void)
{
    if ((amountLeftToOperate == PROGRAM_PHRASE_SIZEOF_INLINE_DATA) ||
        ...
        tr_debug("setupNextProgramData: W8, [%lu]", (uint32_t)currentOperatingStorageAddress);
        ...
        tr_debug("setupNextProgramData: W%u, [%lu]", amount, (uint32_t)currentOperatingStorageAddress);
    ...
    switch (currentCommand) {
        case ARM_STORAGE_OPERATION_PROGRAM_DATA:
            ...
            if (failedWithAccessError() || failedWithProtectionError()) {
                clearErrorStatusBits();
                if (commandCompletionCallback) {
                    tr_debug("irq: [PROGRAM] invoking callback with error");
        ...
        case ARM_STORAGE_OPERATION_ERASE:
            ...
            if (failedWithAccessError() || failedWithProtectionError()) {
                clearErrorStatusBits();
                if (commandCompletionCallback) {
                    tr_debug("irq: [ERASE] invoking callback with error");
