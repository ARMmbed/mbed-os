...
class SecureFunction(object):
    ...
    def __init__(self, sfid, signal, non_secure_clients, minor_version=1, minor_policy='strict'):
        """
        ...
        :param non_secure_clients: True to allow connections from non-secure partitions
        ...
        assert isinstance(non_secure_clients, bool), 'non_secure_clients parameter must be of boolean type'
        ...
        assert minor_policy in self.MINOR_POLICIES, 'minor_policy parameter is invalid'
...
class MmioRegion(object):
    MMIO_PERMISIONS = {'READ-ONLY': 'PSA_MMIO_PERM_READ_ONLY', 'READ-WRITE': 'PSA_MMIO_PERM_READ_WRITE'}
    ...
    def __init__(self, **kwargs):
        ...
        Incase of named region the acceptable params are name and permission
        Incase of numeric region the acceptable params are name, size and permission
        
        :param name: C defention name of the region (size will be auto-generated)
        ...
        self.permission = self.MMIO_PERMISIONS[kwargs['permission']]
        if 'name' in kwargs:
...
class Irq(object):
    ...
    def __eq__(self, other):
        return (self.line_num == other.line_num) and (self.signal == other.signal)


class Manifest(object):
    PRIORITY = {'LOW': 'osPriorityLow', 'NORMAL': 'osPriorityNormal', 'HIGH': 'osPriorityHigh'}
    PARTITION_TYPES = ['SECURE', 'TRUSTED']

    def __init__(self, manifest_file, name, partition_id, partition_type, priority,
                 entry_point, heap_size, stack_size,
                 source_files, mmio_regions=None, secure_functions=None,
                 extern_sfids=None, irqs=None):
        """
        ...
        :param partition_type: Whether the partition is unprivileged or part of the trusted computing base
        ...
        :param source_files: List of files assembling the partition ( Relative paths)
        :param mmio_regions: List of MMIO regions used by the partition
        :param secure_functions: List of Secure functions declared by the partition
        ...
        for src_file in self.source_files:
            assert os.path.isfile(src_file), 'The source file {} mentioned in {} doesn\'t exist.'.format(
                src_file, self.file)
        ...
        assert len(self.extern_sfids) == len(set(self.extern_sfids)), 'Detected duplicates extern SFIDs in {}'.format(
                self.file)
        ...
        assert len(self.secure_functions) + len(self.irqs) > 0,\
            'Manifest {} - {} does not contain at least 1 IRQ or SF'.format(self.name, self.file)
    ...
    def from_json(cls, manifest_file):
        ...
        partition_schema_path = path_join(SCRIPT_DIR, 'partition_description_schema.json')
...
def validate_partition_manifests(manifests):
    """
    Check the correctness of the manifests list (no conflicts, no missing elements, etc.)
    ...
    for manifest in manifests:
        ...
        if manifest.name in partitions_names:
            raise ValueError('Partition name {} is not unique, found in both {} and {}.'.format(
                manifest.name, partitions_names[manifest.name], manifest.file))
        partitions_names[manifest.name] = manifest.file

        # Make sure the partition ID's are unique.
        if manifest.id in partitions_ids:
            raise ValueError('Partition id {} is not unique, found in both {} and {}.'.format(
                manifest.id, partitions_ids[manifest.id], manifest.file))
        ...
        for secure_function in manifest.secure_functions:
            if secure_function.sfid in secure_function_ids:
                raise ValueError('Secure function sfid {} is found in both {} and {}.'.format(
                    secure_function.sfid, secure_function_ids[secure_function.sfid], manifest.file))
            secure_function_ids[secure_function.sfid] = manifest.file

            if secure_function.signal in secure_function_signals:
                raise ValueError('Secure function signal {} is found in both {} and {}.'.format(
                    secure_function.signal, secure_function_signals[secure_function.signal], manifest.file))
        ...
        for irq in manifest.irqs:
            if irq.signal in irq_signals:
                raise ValueError('IRQ signal {} is found in both {} and {}.'.format(
                    irq.signal, irq_signals[irq.signal], manifest.file))
    ...
    for manifest in manifests:
        ...
        if not extern_sfids.issubset(declared_sfids):
            ...
            raise ValueError('External SFID(s) {} required by {} can\'t be found in any partition manifest.'.format(
...
def generate_source_files(manifests, template_files, output_dir, extra_filters=None):
    ...
    partition_templates = filter(lambda filename: '_NAME_' in filename, template_files)
    common_templates = [elem for elem in template_files if elem not in partition_templates]

    for template_file in common_templates:
        template = env.get_template(template_file)
        generated_filename = os.path.splitext(template_file)[0]
        rendered_files[path_join(autogen_folder, generated_filename)] = template.render(
    ...
    for partition in manifests:
        for template_file in partition_templates:
            template = env.get_template(template_file)
            generated_filename = os.path.splitext(template_file.replace('NAME', partition.name.lower()))[0]
            rendered_files[path_join(autogen_folder, generated_filename)] = template.render(partition=partition)
...
def is_up_to_date(manifest_files, template_files, generated_files):
    ...
    num_of_partition_templates = len(list(filter(lambda filename: '_NAME_' in filename, template_files)))
    num_of_common_templates = len(template_files) - num_of_partition_templates
    expected_num_of_generated_files = (num_of_partitions * num_of_partition_templates) + num_of_common_templates

    if expected_num_of_generated_files == len(generated_files) and \
       get_latest_timestamp(*generated_files) > get_latest_timestamp(__file__, *(template_files + manifest_files)):
        return True
    else:
        return False
...
def process_manifest_files(manifest_files, output_dir):
    ...
    manifests = [Manifest.from_json(manifest_file) for manifest_file in manifest_files]

    # Validate the correctness of the manifest collection.
    validate_partition_manifests(manifests)

    # Generate code.
    return generate_source_files(sorted(manifests, key=lambda k: k.name), template_files, output_dir)
