...
from utils import argcomplete_many, argcomplete_list
...
def get_default_options_parser(add_clean=True, add_options=True):
    ...
    parser.add_argument("-m", "--mcu",
                      ...
                      metavar="MCU",
                        type=argparse_many(argparse_force_uppercase_type(targetnames, "MCU"))).completer = argcomplete_many(argcomplete_list(targetnames))

    parser.add_argument("-t", "--tool",
                      help="build using the given TOOLCHAIN (%s)" % ', '.join(toolchainlist),
                      metavar="TOOLCHAIN",
                        type=argparse_many(argparse_force_uppercase_type(toolchainlist, "toolchain"))).completer = argcomplete_many(argcomplete_list(toolchainlist))
    ...
    options_list =['save-asm', 'debug-info', 'analyze']
    if add_options:
        parser.add_argument("-o", "--options", action="append",
                          help='Add a build argument ("save-asm": save the asm generated by the compiler, "debug-info": generate debugging information, "analyze": run Goanna static code analyzer")',
                            type=argparse_lowercase_hyphen_type(options_list, "build option")).completer=argcomplete_list(options_list)
