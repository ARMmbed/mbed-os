...
import tools.spm.generate_partition_code as generate_partition_code  # Imported again as a module for monkey-patching
...
def dump_manifest_to_json(manifest, test_name, test_dir, create_files=True):
    ...
    :param create_files: Whether to create the source files listed in the manifest 'source_files' entry.
...
def find_priority_key(value):
    ...
    """
    return next((key for key, val in Manifest.PRIORITY.items() if val == value), None)
...
def find_permission_key(value):
    ...
    :return: The key of the given value.
    """
    return next((key for key, val in MmioRegion.MMIO_PERMISIONS.items() if val == value), None)


@pytest.fixture(scope="session")
def temp_test_data(tmpdir_factory):
    """
    Fixture (https://docs.pytest.org/en/latest/fixture.html) function to be used by the tests.
    This fixture function Creates a valid JSON manifest file in a temporary directory.
    The scope of this fixture is the entire test session.
...
"""
'modified_json_params' contain the parameters to be used in the 'modified_json' fixture.
Each key in the dictionary represents a different parameter to be used by 'modified_json', so for each test which uses
the 'modified_json' fixture, the test will run len(modified_json_params) times, each time with different parameters.
Each parameter is a dictionary which contains these keys:
  'partition': A modified partition dictionary.
  'assert': The expected assertion which must occur when running with this parameter.
"""
modified_json_params = {
    ...
    'invalid_mmioregion_base': {
        'partition': dict(manifests[0], mmio_regions=[invalid_mmioregion_base]),
        'assert': jexcep.ValidationError
    },
    'invalid_mmioregion_size': {
        'partition': dict(manifests[0], mmio_regions=[invalid_mmioregion_size]),
        'assert': jexcep.ValidationError
    },
    'invalid_irq_num': {
...
def modified_json(request, temp_test_data):
    """
    Fixture (https://docs.pytest.org/en/latest/fixture.html) function to be used by the tests.
    This fixture function Creates a JSON manifest file from a given partition dictionary and save it
    to a temporary directory.
    This fixture uses the 'temp_test_data' fixture.
    This fixture is a parametrized fixture (https://docs.pytest.org/en/latest/fixture.html#parametrizing-fixtures).
    The scope of this fixture is a specific test.

    :param request: Request object which contain the current parameter from 'modified_json_params'.
...
def test_invalid_json(modified_json):
    """
    Test which gets an invalid JSON manifest file (from the 'modified_json' fixture) and tries to create a
...
def test_valid_json(temp_test_data):
    """
    Test which gets a valid JSON manifest file (from the 'temp_test_data' fixture) and tries to create a
    Manifest object from it.
    The test expects the Manifest to be same as the Manifest created by the 'temp_test_data' fixture.
...
# Contain the parameters to be used in the 'test_validate_partition_manifest' test.
# It defines a list of (manifest, assertion) tuples which each entry will be the input of the
# 'test_validate_partition_manifest' test, the test will run len(LIST_OF_TUPPLES) times,
# each time with different (manifest, assertion) tuple.
# The tuple fields are:
#   'manifest': A modified partition dictionary.
#   'assertion': A tuple containing the expected assertion and assertion string which must occur when running
#                with this parameter.
...
def test_validate_partition_manifest(request, temp_test_data, manifest, assertion):
    """
    Test which creates an invalid manifest object (after passing JSON schema validation) and call
    validate_partition_manifests() with it and with a valid manifest object.
    The test expects an assertion to happen.

    :param request: Request object.
    :param temp_test_data: The 'temp_test_data' fixture.
    :param manifest: The manifest value from the (manifest, assertion) tuple for the current parameter.
    :param assertion: The assertion value from the (manifest, assertion) tuple for the current parameter.
    :return:
    """
...
"""
'verify_json_params' contain the parameters to be used in the 'verify_json' fixture.
Each key in the dictionary represents a different parameter to be used by 'verify_json', so for each test which uses
the 'verify_json' fixture, the test will run len(verify_json_params) times, each time with different parameters.
Each parameter is a dictionary which contains these keys:
  'partition': A modified partition dictionary.
...
verify_json_params = {
    'missing_minor_version_secure_functions': {
        ...
        'expected': [
            SecureFunction(
                sfid='SFID1', signal='SFID1', minor_policy='relaxed', non_secure_clients=True, minor_version=1
            )
        ]
    },
    'missing_minor_version_policy_secure_functions': {
        ...
        'expected': [
            SecureFunction(
                sfid='SFID2', signal='SFID2', minor_policy='strict', non_secure_clients=True, minor_version=1
            )
        ]
    },
    'missing_minor_completley_secure_functions': {
        ...
        'expected': [
            SecureFunction(
                sfid='SFID2', signal='SFID2', minor_policy='strict', non_secure_clients=False, minor_version=1
            )
        ]
    }
}
...
def verify_json(request, tmpdir_factory):
    """
    Fixture (https://docs.pytest.org/en/latest/fixture.html) function to be used by the tests.
    This fixture function Creates 2 JSON manifest files (The 1st from 'verify_json_params', the 2nd from manifests[1])
    and saves them to a temporary directory.
    This fixture is a parametrized fixture (https://docs.pytest.org/en/latest/fixture.html#parametrizing-fixtures).
    The scope of this fixture is a specific test.

    :param request: Request object which contain the current parameter from 'verify_json_params'.
    ...
    return {'files_list': files_list, 'field': request.param['field'], 'expected': request.param['expected']}


def test_verify_json(verify_json):
    """
    Test which gets 2 JSON manifest files (from the 'verify_json' fixture), create Manifest objects from them, call
    validate_partition_manifests() on the manifest objects and check that the 1st Manifest object is as expected.
...
# It defines a list of (mock_files, expected) tuples which each entry will be the input of the
# 'test_is_up_to_date' test, the test will run len(LIST_OF_TUPPLES) times,
# each time with different (mock_files, expected) tuple.
# The tuple fields are:
#   'mock_files': A dictionary to mock file mofied times.
#                 Each entry key is the name of the file and value is it's modified time.
#   'expected': Expected return value of is_up_to_date() with the given mock_files.
@pytest.mark.parametrize(
    'mock_files, expected',
    [
        pytest.param(test_mock_files, True, id='general'),
        pytest.param(
            {k: test_mock_files for k in test_mock_files if not k.startswith('manifest')}, False, id='no_manifest_files'
        ),
        pytest.param(
            {k: test_mock_files for k in test_mock_files if not k.startswith('template')}, False, id='no_template_files'
        ),
        pytest.param(
            {k: test_mock_files for k in test_mock_files if not k.startswith('gen')}, False, id='no_generated_files'
        ),
        pytest.param(
            dict(test_mock_files, manifest1=max(test_mock_files.values()) + 1), False, id='manifest_changed'
        ),
        pytest.param(
            dict(test_mock_files, template_common1=max(test_mock_files.values()) + 1), False, id='template_changed'
        )
    ]
)
def test_is_up_to_date(monkeypatch, mock_files, expected):
    """
    Test which calls is_up_to_date() with a given list of files ('mock_files' keys) and checks that the return value is
    same as the 'expected' value.
    The separation of the files in 'mock_files' to (manifest_files, template_files, generated_files) is achieved by
    their name's prefix, for example: list of template files is  all the keys in 'mock_files' with 'gen' prefix.

    :param monkeypatch: The 'monkeypath' fixture (https://docs.pytest.org/en/latest/monkeypatch.html).
    :param mock_files: The mock_files value from the (mock_files, expected) tuple for the current parameter.
    :param expected: The expected value from the (mock_files, expected) tuple for the current parameter.
    :return:
    """
    def mock_getmtime(mock_file):
        """
        A mocking function for 'os.path.getmtime' which returns the value of 'test_mock_files[mock-file]'

        :param mock_file: The mocked file name (key in 'test_mock_files').
        :return: The mocked modification-time (value of 'test_mock_files[mock-file]').
        """
    ...
    def get_mock_files(prefix):
        """
        Returns a subset list from 'test_mock_files' with key that starts with 'prefix'.
...
def test_template_setup(tmpdir_factory):
    """
    Fixture (https://docs.pytest.org/en/latest/fixture.html) function to be used by the tests.
    This fixture function Creates JSON manifest files, Manifest objects from 'manifest' and template files
    ...
    :return: A dictionary containing these keys:
             ...
             'filters': Dictionary with additional filters for generate_source_files()
    """
    def find_priority_key(value):
        """
        Finds the key in 'Manifest.PRIORITY' of a given value.
    ...
    test_dir = tmpdir_factory.mktemp('test_data')
    manifest_files = [dump_manifest_to_json(manifest, manifest['name'], test_dir) for manifest in manifests]
    ...
    template_files = [test_dir.join('_NAME_.json.tpl'), test_dir.join('common.json.tpl')]
    for template, _file in [(test_partition_template, template_files[0]), (test_common_template, template_files[1])]:
...
def test_generate_source_files(test_template_setup):
    """
    Test which calls generate_source_files() with the data from 'test_template_setup' fixture and checks normal output.
    ...
    for gen_file in [os.path.join(autogen_dir, f) for f in os.listdir(autogen_dir)]:
        """
        ...
        if 'name' in generated:
            input_file = os.path.join(test_template_setup['dir'], generated['name'] + '.json')
            assert os.path.isfile(input_file)
            assert input_file in test_template_setup['manifest_files']
            with open(input_file) as fh:
                expected = json.load(fh)
        else:
            region_list = [region for manifest in test_template_setup['manifests'] for region in manifest.mmio_regions]
            expected = {
                 'num_of_partitions': len(test_template_setup['manifests']),
                 'partition_names': [manifest.name for manifest in test_template_setup['manifests']],
                 'num_of_region_pairs': len(list(itertools.combinations(region_list, 2)))
            }
...
def test_process_manifest_files(monkeypatch, test_template_setup):
    """
    Test which calls process_manifest_files() with the data from 'test_template_setup' fixture.
    Because process_manifest_files() is a compound of the other functions in the module which are tested individually,
    this test just do the following:
    1. Calls process_manifest_files() and checks that the autogen directory was created.
    2. Saves the modified times of the generated files.
    3. Calls process_manifest_files() again, checks that the autogen directory still exist and that modified times
       of the generated files didn't change.

    :param monkeypatch: The 'monkeypath' fixture (https://docs.pytest.org/en/latest/monkeypatch.html).
    :param test_template_setup: The 'test_template_setup' fixture.
    :return:
    """
    monkeypatch.setattr(generate_partition_code, 'TEMPLATES_DIR', test_template_setup['dir'])
    monkeypatch.setitem(DEFAULT_FILTERS, 'basename', os.path.basename)
    monkeypatch.setitem(DEFAULT_FILTERS, 'find_priority_key', find_priority_key)
    monkeypatch.setitem(DEFAULT_FILTERS, 'find_permission_key', find_permission_key)

    autogen_dir = process_manifest_files(test_template_setup['manifest_files'], test_template_setup['dir'])
    assert os.path.isdir(autogen_dir)

    autogen_files = {}
    for file_name in os.listdir(autogen_dir):
        autogen_files[file_name] = os.path.getmtime(os.path.join(autogen_dir, file_name))

    autogen_dir = process_manifest_files(test_template_setup['manifest_files'], test_template_setup['dir'])
    assert os.path.isdir(autogen_dir)

    for file_name in os.listdir(autogen_dir):
        assert autogen_files[file_name] == os.path.getmtime(os.path.join(autogen_dir, file_name))