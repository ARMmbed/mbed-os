...
MODULE TEST GENERATE NEW MODULE TESTS
"""
...
class ModuleTestGeneratorTool(object):
    """
    Generator tool to create new module tests from template
    """
    ...
    def create(self, mbed_os_root=None, filepath=None):
        """
        Generate a module test for a file to be tested from templates
        ...
        if filepath is None:
            logging.error("No filepath given. Cannot create a new module test.")
            return
        ...
        test_dir = os.path.join(mbed_os_root, "MODULETESTS", dir_name, class_name)

        if not os.path.exists(test_dir):
            os.makedirs(test_dir)
        else:
            logging.error("Module tests exist already for this file.")
            return
        ...
        try:
            ...
            template_source_file = \
                os.path.join(mbed_os_root,
                             "MODULETESTS/template/test_template.cpp")
            ...
            template_source_file = \
                os.path.join(mbed_os_root,
                             "MODULETESTS/template/moduletest.cmake.template")

            with open(template_source_file, "r") as input_file:
                template_content = input_file.read()

            output_source_file = os.path.join(test_dir, "moduletest.cmake")
            ...
            logging.info("""
Module test %s created.

Generated files into %s directory.
""", suite_name, test_dir)

        except IOError:
            logging.error("Cannot create files for a module test.")
