...
bool USBTester::callback_request(const setup_packet_t *setup) {
    ...
    if (setup->bmRequestType.Type == VENDOR_TYPE) {
        switch (setup->bRequest) {
            case VENDOR_TEST_CTRL_IN:
                ...
                complete_request(ctrl_buf, size, SEND);
                success = true;
                break;
            case VENDOR_TEST_CTRL_OUT:
                size = setup->wValue < 8 ? setup->wValue  : 8;
                complete_request(ctrl_buf, size, RECEIVE);
                success = true;
                break;
            ...
            case VENDOR_TEST_CTRL_NONE_DELAY:
                queue->call_in(2000, static_cast<USBDevice*>(this), &USBTester::complete_request, true);
                success = true;
                break;
            default:
                break;
