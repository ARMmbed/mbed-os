...
//todo: remove uint32_t cfstore_optLogLevel_g = CFSTORE_LOG_DEBUG; /*CFSTORE_LOG_NONE|CFSTORE_LOG_ERR|CFSTORE_LOG_DEBUG|CFSTORE_LOG_FENTRY */
...
#elif defined CFSTORE_CONFIG_MBED_OS_VERSION && CFSTORE_CONFIG_MBED_OS_VERSION == 4
    // todo: put in morpheus equivalent here
#endif /* CFSTORE_CONFIG_MBED_OS_VERSION == x */
 ...
 */
// todo: restore
#ifdef noFuncUse_YOTTA_CFG_CFSTORE_UVISOR
...
static int32_t cfstore_validate_key_name_ex(const char* key_name, const char* permissible)
{
    ...
    if(key_name != NULL){
        ...
        // todo: delete debug
        CFSTORE_DBGLOG("%s:key_name=%s, pos=%s, pos=%p.\n", __func__, key_name, pos, pos);
        while(pos != NULL && brace_count >= 0 && brace_count <= 1)
        {
            CFSTORE_DBGLOG("%s:pos=%s, pos=%p, brace_count=%d, *pos=%c.\n", __func__, pos, pos, (int) brace_count, *pos);
        	switch(*pos)
        	{
        	case '{':
        		brace_count++;
        ...
        if(brace_count != 0){
            CFSTORE_ERRLOG("%s:Unmached brace found in key_name (count=%" PRId32 ".\n", __func__, brace_count);
            return ARM_CFSTORE_DRIVER_ERROR_INVALID_KEY_NAME;
        }
    }
    return ARM_DRIVER_OK;
