	...
	ARM_CFSTORE_KEYDESC *keyDesc)
{
	...
	/* the next line is replaced with 2 lines:              // todo: remove this line after Motti review
	 * TEST_ASSERT_EQUAL(ARM_DRIVER_OK, cfsStatus);         // todo: remove this line after Motti review
	 *
    ...
    /* the next line is updated with new lines:             // todo: remove this line after Motti review
     * TEST_ASSERT_EQUAL(*dataLength, valueLength);         // todo: remove this line after Motti review
     *
     */
...
/* not used: out: */                                        // todo: remove this line after Motti review
	drv->Close(hkey);
...
static control_t cfstore_example2_test_01(const size_t call_count)
{
	...
	/* TEST_ASSERT_TRUE(ARM_DRIVER_OK <= cfsStatus);        // todo: remove this line after Motti review
	 */
     ...
     *  TEST_ASSERT_EQUAL(ARM_DRIVER_OK, cfsStatus);        //return value from Read() (cfsStatus) is not always ARM_DRIVER_OK.                         // todo: remove this line after Motti review
     *  TEST_ASSERT_EQUAL(PvStrLen(value) + 1, len);        // if read successfully, len should equally the valueLen supplied to Create()               // todo: remove this line after Motti review
     *  TEST_ASSERT_EQUAL_HEX8_ARRAY(value, readBuf, len);                                                                                              // todo: remove this line after Motti review
     *
     */
	...
	/*  TEST_ASSERT_EQUAL(ARM_DRIVER_OK, cfsStatus);                        // todo: remove this line after Motti review
	 *  TEST_ASSERT_EQUAL(sizeof(newDataToWrite), len);                     // todo: remove this line after Motti review
	 */
    ...
    /* // todo: remove comment after Motti review
     * Question :
     * <==!!  why suppling a bigger length (more than needed) does NOT cause the origin value filed to shrink? please follow the next note.
     * Answer:
     * - because the KV blob size stays fixed with the value specified to the Create() call i.e. 15
     * - because the code hasnt changed the KV value size using the described procedure i.e.:
     *      - calling Create() on a pre-existing KV but with a NULL key descriptor arg, but with the new length of the value blob.
     *
     */
	len = CFSTORE_KEY_NAME_MAX_LENGTH; // <==!!  why suppling a bigger length (more than needed) does NOT cause the origin value filed to shrink? please follow the next note.      // todo: remove comment after Motti review
	PvMemSet(readBuf, 0, len);
	cfsStatus = drv->Read(updatedKeyH, readBuf, &len);
	/*
	 *  TEST_ASSERT_EQUAL(ARM_DRIVER_OK, cfsStatus);                                                                                                                                    // todo: remove this line after Motti review
	 *  TEST_ASSERT_EQUAL(sizeof(newDataToWrite), len);   // <==!! this fails, sizeof(newDataToWrite) is 11 bytes while "len" is 15 bytes. I expect "len" to be 11 bytes.               // todo: remove this line after Motti review
	 *  TEST_ASSERT_EQUAL_HEX8_ARRAY(newDataToWrite, readBuf, len);                                                                                                                     // todo: remove this line after Motti review
	 */
    CFSTORE_TEST_UTEST_MESSAGE(cfstore_example2_utest_msg_g, CFSTORE_UTEST_MSG_BUF_SIZE, "%s:Error: Read() returned value (%" PRId32 ") does not match the created length of the value blob(%" PRId32 ")\n", __func__, cfsStatus, (int32_t) PvStrLen(value) + 1);
    TEST_ASSERT_MESSAGE(cfsStatus == (int32_t) (PvStrLen(value) + 1), cfstore_example2_utest_msg_g);
