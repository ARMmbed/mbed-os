...
uint16_t analogin_read_u16(analogin_t *obj)
{
    MBED_ASSERT(obj);
    uint32_t result = 0;
    adc_get_interrupt_status(ADC); /*Dummy read of interrupt status*/
    result = adc_channel_get_value(ADC, obj->channel); /*Dummy read of current value*/
    adc_start_software_conversion(ADC);
    while (adc_get_interrupt_status(ADC) & (1 << obj->channel));
    result = adc_channel_get_value(ADC, obj->channel);
    return (uint16_t)((result * 0xFFFF) / 0x0FFF);  /* for normalizing to 16 bit value */
}
